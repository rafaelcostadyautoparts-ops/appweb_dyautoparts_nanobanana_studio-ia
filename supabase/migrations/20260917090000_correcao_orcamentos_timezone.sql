...
-- 1. Atualizar defaults das tabelas para armazenar o instante real UTC (now())
ALTER TABLE public.orcamentos
    ALTER COLUMN criado_em SET DEFAULT now(),
    ALTER COLUMN atualizado_em SET DEFAULT now(),
    ALTER COLUMN emissao SET DEFAULT (timezone('America/Sao_Paulo', now())::date);

ALTER TABLE public.orcamento_versoes
    ALTER COLUMN criado_em SET DEFAULT now();

-- 2. Atualizar RPC salvar_rascunho_orcamento para usar now() diretamente para timestamptz
...
DECLARE
    ...
    v_emissao date := coalesce((p_condicoes->>'emissao')::date, (now() AT TIME ZONE 'America/Sao_Paulo')::date);
...
-- 3. Atualizar RPC gerar_versao_oficial_orcamento para usar now() diretamente para timestamptz
...
DECLARE
    v_rec public.orcamentos%ROWTYPE;
    v_today_str text := to_char(now() AT TIME ZONE 'America/Sao_Paulo', 'YYYYMMDD');
    v_now timestamp with time zone := now();
    v_emissao date := coalesce((p_condicoes->>'emissao')::date, (now() AT TIME ZONE 'America/Sao_Paulo')::date);
    v_validade date := coalesce((p_condicoes->>'validade')::date, v_emissao + integer '7');
    v_usuario text := coalesce(nullif(btrim(coalesce(p_usuario, '')), ''), 'N/A');
