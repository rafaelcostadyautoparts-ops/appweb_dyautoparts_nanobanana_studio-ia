-- Migration: 20260913120000_conferencia_local_origem_real.sql
-- FASE 2A.1: Correção da Baixa de Estoque da Conferência usando local_origem Real das Bipagens Físicas

BEGIN;

-- 1. RPC de Finalização com Baixa de Estoque por local_origem Físico
...
-- 2. RPC de Cancelamento de Separação (Estorno Rastreável pelo local_origem dos Movimentos)
...
BEGIN
  ...
  -- Estorna saídas operacionais desta separação usando os locais de origem reais gravados nos movimentos
  FOR v_mov IN
    SELECT m.*
      FROM public.movimentos m
     WHERE lower(coalesce(m.tipo, '')) = 'saida'
       AND (lower(coalesce(m.origem, '')) IN ('conferencia', 'app_conferencia', 'app_separacao')
  ...
  LOOP
    ...
    v_local_estorno := upper(btrim(coalesce(v_mov.local_origem, '')));
    IF v_local_estorno IS NULL OR v_local_estorno = '' THEN
      RAISE EXCEPTION 'Movimento % nao possui local_origem valido para estorno.', v_mov.movimento_id;
    END IF;

    SELECT id INTO v_stock_id
      FROM public.estoque_atual
     WHERE id_interno = v_mov.id_interno
       AND upper(btrim(local)) = v_local_estorno
     ORDER BY id
     LIMIT 1
     FOR UPDATE;
    ...
    IF v_stock_id IS NULL THEN
      ...
      VALUES(v_mov.id_interno, v_local_estorno, v_mov.quantidade, 0, 0, v_mov.quantidade, v_now, v_mov.id_interno || '|' || v_local_estorno)
    ...
    VALUES(v_estorno_id, v_now, 'entrada', v_mov.id_interno, NULL, v_local_estorno, v_mov.quantidade,
...
COMMENT ON FUNCTION public.cancelar_separacao_antes_despacho(text, text, text)
IS 'Estorna os movimentos de saida gerados pela conferencia devolvendo o estoque para o local_origem exato de onde sairam.';
