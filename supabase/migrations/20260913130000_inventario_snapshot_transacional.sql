-- Migration: 20260913130000_inventario_snapshot_transacional.sql
-- FASE 2B.3/2B.4: Snapshot Transacional Definitivo do Inventario (Evolucao de Banco)

BEGIN;

-- 1. Garantir colunas de snapshot em inventarios_itens
...
-- 2. RPC Transacional para Salvar Contagem Fisica e Congelar Snapshot
...
BEGIN
    ...
    -- 1. Validar se existe bipagem ativa com local_origem NULL/vazio para ESTE SKU FISICO especificamente
    SELECT EXISTS (
        SELECT 1
         ...
         WHERE (b.id_interno = v_id_interno OR b.produto_id = (SELECT id FROM public.produtos WHERE id_interno = v_id_interno LIMIT 1))
           AND (b.local_origem IS NULL OR btrim(b.local_origem) = '')
           AND s.status IN ('em_separacao', 'separado', 'em_conferencia')
    ) INTO v_has_null_origin;

    IF v_has_null_origin THEN
        RAISE EXCEPTION 'Existe produto em separacao sem local de origem definido. Regularize a separacao antes de confirmar esta contagem.';
    END IF;
    ...
    ELSE
        SELECT coalesce(preco_custo, 0) INTO v_valor_unitario
    ...
    RETURN jsonb_build_object(
        ...
        'diferenca', v_diferenca,
        'valor_diferenca', v_diferenca * coalesce(v_valor_unitario, 0),
        'auditado_em', v_now,
        'auditado_por', btrim(p_usuario)
    );
END;
...
-- 3. RPC de Finalizacao do Inventario com base Exclusiva no Snapshot Congelado
...
BEGIN
    ...
    LOOP
        ...
        -- 1. Exigir Snapshot de Q_sep Valido (q_sep_snapshot_em nao pode ser NULL)
        IF v_item.q_sep_snapshot_em IS NULL THEN
            RAISE EXCEPTION 'Produto sem snapshot de separacao valido. Reconfirme a contagem antes de finalizar o inventario.';
        END IF;
        ...
        SELECT EXISTS (
            SELECT 1
             ...
             WHERE m.id_interno = v_item.id_interno
               ...
               AND coalesce(m.origem, '') NOT IN ('ajuste_inventario', 'inventario', 'APP_INVENTARIO')
               AND coalesce(m.execution_id, '') NOT LIKE (v_execution_id || '%')
        ) INTO v_has_post_mov;

        IF v_has_post_mov THEN
            RAISE EXCEPTION 'CONTAGEM DESATUALIZADA — houve movimentacao deste produto apos a contagem. Refaca a contagem antes de finalizar.';
        END IF;
        ...
        IF v_diferenca <> 0 THEN
            v_item_execution_id := v_execution_id || '-' || v_item.id::text;
