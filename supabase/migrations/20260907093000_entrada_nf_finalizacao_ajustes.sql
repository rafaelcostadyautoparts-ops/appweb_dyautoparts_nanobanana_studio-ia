-- Migration: BLOCO 2 - Ajustes de Rastreabilidade e Refinamento de Custo Real
-- Data: 2026-09-07

-- 1. Expansão de schema em public.estoque_lotes para vinculo direto com a alocacao fisica (recebimento_id)
ALTER TABLE public.estoque_lotes
    ADD COLUMN IF NOT EXISTS recebimento_id uuid NULL;

CREATE INDEX IF NOT EXISTS idx_estoque_lotes_recebimento_id
ON public.estoque_lotes (recebimento_id);

-- 2. Atualizacao incremental da RPC transacional de finalizacao
...
BEGIN
    ...
    LOOP
        ...
        IF v_qtd_aceita > 0 THEN
            ...
            -- Regra de custo unitario real pre-recusa:
            -- 1. Utiliza custo_real_unitario da linha XML (custo real total / qtd XML, com frete, impostos, rateios).
            -- 2. Senao calcula custo_real_total / quantidade_xml.
            -- 3. Senao utiliza valor_unitario fiscal do XML.
            ...
            -- 5b. Inserir movimento em public.movimentos (com execution_id = entrada_nf_rec:<recebimento_id>)
            ...
            -- 5c. Criar camada de Lote FIFO em public.estoque_lotes (com recebimento_id para rastreabilidade 1:1)
