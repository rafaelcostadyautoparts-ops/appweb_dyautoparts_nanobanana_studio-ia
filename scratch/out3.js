...
function getStoragePath(file, tipo = "imagem") {
  const name = file.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, "-").replace(/[^a-z0-9.-]/g, "");
...
function getPublicUrl(path) {
  if (!path) return null;
  const { data } = window.supabaseClient.storage.from("assets").getPublicUrl(path);
