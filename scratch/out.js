...
function playFeedbackSound(type) {
  try {
    ...
    console.warn("[AUDIO] feedback indispon\xEDvel", err);
  }
}
...
async function handleProductScan(rawValue, context = "search") {
  ...
  console.log(`[SCAN] Classifica\xE7\xE3o:`, classification);
  ...
  if (classification.type === "invalid" || classification.type === "empty") {
    showScanFeedback("error", "C\xF3digo Inv\xE1lido");
    return;
  }
  ...
  if (product) {
    ...
    if (context !== "search") {
      showScanFeedback("warning", "Produto n\xE3o cadastrado");
    }
    return null;
  }
}
...
window.addEventListener("popstate", (event2) => {
  if (event2.state && event2.state.screen) {
    console.log("[NAV] Popstate para:", event2.state.screen);
    renderScreenByName(event2.state.screen, false);
  } else if (initialized) {
    renderMenu(false);
  }
});
...
function renderScreenByName(name, push = true) {
  switch (name) {
    case "menu":
      ...
      break;
    case "search":
      renderSearchScreen(push);
      break;
    case "login":
      renderLogin(push);
      break;
    case "config":
      renderConfigSubMenu(push);
      break;
    default:
      renderMenu(push);
  }
...
window.handleUserClick = function(e) {
  ...
  showToast("CLICK USUARIO OK", "success");
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (footer) {
    ...
    if (menuScreen) {
      menuScreen.classList.add("menu-footer-visible");
    }
  }
      ...
      reg.addEventListener("updatefound", () => {
        ...
        newWorker.addEventListener("statechange", () => {
          if (newWorker.state === "installed" && navigator.serviceWorker.controller) {
            if (confirm("Nova vers\xE3o dispon\xEDvel. Atualizar?")) {
...
function addSyncTrace(origin, action, details = "") {
  ...
  const entry = {
    time: (/* @__PURE__ */ new Date()).toISOString().substr(11, 12),
    origin,
    action,
    details,
...
async function copyToClipboard(text, elementId = null) {
  try {
    ...
    showToast("Erro ao copiar para a \xE1rea de transfer\xEAncia.");
  }
}
function toggleAllStock() {
  ...
  if (grid && btn) {
    ...
    btn.innerHTML = isShowingAll ? '<span class="material-symbols-rounded">expand_less</span> VER MENOS' : '<span class="material-symbols-rounded">expand_more</span> VER TODOS';
  }
}
...
async function verificarConexao() {
  try {
    ...
    console.error("Erro fatal ao verificar conex\xE3o:", err);
    return false;
  }
}
...
function normalizeSheetValue(sheet, field, value) {
  ...
  if (sheet === "movimentos") {
    if (field === "tipo") {
      ...
      return valid.includes(lower) ? lower : "saida";
    }
  ...
  try {
    ...
    await fetch(SCRIPT_URL, {
      ...
      body: JSON.stringify(normalizePayloadForSheet({
        action: "movimento",
        tipo: "saida",
        // Normalizado para 'saida' (estorno é uma saída de correção)
        ...
        observacao: `Corre\xE7\xE3o de erro operacional da sessao ${sessionId}`
      }))
    });
    showToast(`Estorno conclu\xEDdo (sincronizado).`);
  } catch (err) {
...
function formatImageUrl(url) {
  ...
  if (url.startsWith("produtos/") || url.startsWith("branding/")) {
    try {
      ...
      console.log("[formatImageUrl] Erro ao gerar URL p\xFAblica:", e);
    }
  }
  if (url.includes("drive.google.com")) {
    const match = url.match(/\/d\/([a-zA-Z0-9_-]+)/) || url.match(/id=([a-zA-Z0-9_-]+)/) || url.match(/\/file\/d\/([a-zA-Z0-9_-]+)/);
    if (match && match[1]) {
...
const attributeNameMap = {
  voltagem: "Voltagem",
  potencia: "Pot\xEAncia",
  tipo_lampada: "Tipo da l\xE2mpada",
  temperatura_cor: "Temperatura de cor",
  lumens: "Lumens",
  encaixe: "Encaixe",
  codigo_equivalente: "C\xF3digo equivalente",
  linha: "Linha",
  ip_rate: "IP Rate",
  chip_led: "Chip LED",
  ...
  material_carcaca: "Material da carca\xE7a",
  regulagem: "Regulagem",
  modelo_botao: "Modelo do bot\xE3o",
  cor_botao: "Cor do bot\xE3o",
  veiculo: "Ve\xEDculo",
  ano_aplicacao: "Ano de aplica\xE7\xE3o"
};
...
function handleImageError(img, fallbackIcon = "directions_car") {
  img.onerror = null;
  img.src = "/imagens/icon-512-black.png";
...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbznHLTXr_--3PrR8GAz4-TrtX4jttC5cg7CH8cPa7KzoRQPQMZrmtEPBAMWE5KqMTUXwA/exec";
...
const DATA_MAX_AGE_MS = 5 * 60 * 1e3;
...
async function ensureFreshData(callback) {
  ...
  addTechnicalLog("SYNC_CHECK", "STALE", "Dados antigos. Disparando sync obrigat\xF3ria...");
  const success = await loadAllData(false);
  if (success) {
    callback();
  } else {
    showToast("Falha na sincroniza\xE7\xE3o. Verifique sua conex\xE3o.");
  }
}
...
const BOOT_CONFIG = {
  TIMEOUT_MS: 1e4,
  // 10s timeout por etapa
  MAX_RETRIES: 1,
  // máximo 1 retry
  BOOT_TIMEOUT_MS: 3e4
  // 30s timeout total do bootstrap
};
let bootstrapState = {
  running: false,
  completed: false,
...
async function initApp() {
  ...
  if (bootstrapState.running) {
    console.log("[BOOT] Reentrada bloqueada - bootstrap j\xE1 em execu\xE7\xE3o");
    addSyncTrace("initApp", "BLOCK", "reentrada");
    return;
  }
  if (bootstrapState.completed) {
    console.log("[BOOT] Reentrada bloqueada - bootstrap j\xE1 conclu\xEDdo");
    addSyncTrace("initApp", "BLOCK", "j\xE1 completado");
    return;
  }
  ...
  console.log("[BOOT] ==========================================");
  console.log("[BOOT] IN\xCDCIO DO BOOTSTRAP");
  console.log("[BOOT] ==========================================");
  const totalTimeout = setTimeout(() => {
    if (bootstrapState.running && !bootstrapState.completed) {
      console.error("[BOOT] TIMEOUT TOTAL DE INICIALIZA\xC7\xC3O");
      bootstrapState.abortController.abort();
      showBootstrapError("Timeout de inicializa\xE7\xE3o");
    }
  ...
  try {
    ...
    try {
      ...
      console.warn("[BOOT] Supabase n\xE3o dispon\xEDvel:", se.message);
    }
    atualizarStatusConexao();
    console.log("[BOOT] Carregando usu\xE1rios...");
    const usersLoaded = await loadUsersWithFallback();
    if (usersLoaded) {
      console.log(`[BOOT] Usu\xE1rios carregados: ${appData.users?.length || 0}`);
    } else {
      console.warn("[BOOT] Usu\xE1rios n\xE3o carregados, usando fallback");
    }
    const currentUser = localStorage.getItem("currentUser");
    if (!currentUser) {
      console.log("[BOOT] Carregando dados m\xEDnimos (separacao)...");
      try {
        await withTimeout(
    ...
    console.log("[BOOT] ==========================================");
    console.log(`[BOOT] BOOTSTRAP CONCLU\xCDDO (${elapsed}ms)`);
    console.log("[BOOT] ==========================================");
    addSyncTrace("initApp", "COMPLETE", `sucesso em ${elapsed}ms`);
  } catch (err) {
    clearTimeout(totalTimeout);
    console.error("[BOOT] Erro cr\xEDtico no bootstrap:", err);
    addSyncTrace("initApp", "ERROR", err.message);
    hideSplash();
    try {
      renderLogin();
    } catch (e2) {
      console.error("[BOOT] Tamb\xE9m falhou renderLogin:", e2);
    }
    bootstrapState.completed = true;
    bootstrapState.running = false;
  }
...
async function loadUsersWithFallback() {
  console.log("[BOOT] Carregando usu\xE1rios do Supabase...");
  try {
    ...
    if (data && data.length > 0) {
      appData.users = data.map((u) => ({
        ...u,
        avatar_url: u.avatar_url && !["sim", "nao", "n\xE3o"].includes(String(u.avatar_url).toLowerCase()) && (String(u.avatar_url).startsWith("http") || String(u.avatar_url).startsWith("data:")) ? u.avatar_url : ""
      }));
      console.log(`[DATA] usuarios -> Supabase (${appData.users.length} usu\xE1rios)`);
      console.log(`[DATA] Google Sheets ignorado para 'usuarios'`);
      return true;
    }
    console.error("[BOOT] Supabase retornou vazio para usu\xE1rios (SSOT FALHOU)");
  } catch (e) {
    console.error(`[BOOT] Erro cr\xEDtico ao carregar usu\xE1rios do Supabase: ${e.message}`);
  }
  addSyncTrace("loadUsersWithFallback", "ABORT", "Supabase falhou e Sheets est\xE1 desativado como fallback");
  return false;
}
...
const PROCESS_SYNC_QUEUE_DEBOUNCE_MS = 3e3;
...
async function getQueuedOperations() {
  ...
  const fallback = JSON.parse(localStorage.getItem("operation_outbox_fallback") || "[]");
  return [...operations, ...fallback].filter((op) => op.status === "pending" || op.status === "error").sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
}
async function markQueuedOperation(operation, status, error = "") {
  ...
  try {
    ...
    const fallback = JSON.parse(localStorage.getItem("operation_outbox_fallback") || "[]").filter((op) => op.id !== operation.id);
...
async function processSyncQueue(caller = "unknown") {
  ...
  if (now - lastProcessSyncQueueCall < PROCESS_SYNC_QUEUE_DEBOUNCE_MS) {
    console.log(`[SYNC] processSyncQueue ignorado (debounce): ${caller} | \xDAltima: ${now - lastProcessSyncQueueCall}ms`);
  ...
  try {
    ...
    if (queue.length === 0 && itemsProcessed > 0) {
      ...
      console.log(`[SYNC] Fila j\xE1 estava vazia. Nenhuma carga adicional necess\xE1ria.`);
...
async function safePost(payload) {
  ...
  try {
    ...
    console.log("[safePost] Envio conclu\xEDdo com sucesso (mode: no-cors)");
    return true;
  } catch (error) {
    console.error("[safePost] Erro na requisi\xE7\xE3o:", error);
    await queueOperation("google_sheets_post", syncItem.payload, { backupTarget: "google_sheets" });
    if (error.name === "TypeError" && error.message.includes("fetch")) {
      showToast("Erro de conex\xE3o: Salvando em fila local.");
    } else if (error.name === "AbortError") {
      showToast("Timeout: Opera\xE7\xE3o cancelada. Salvando em fila.");
    } else {
...
async function fetchSheetData(sheetName, timeoutMs = 2e4) {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      ...
      showToast(`Erro de conex\xE3o ao buscar '${sheetName}'`, "error");
    } else {
...
function handleActionKey(event2, action) {
...
function getTopBarHTML(currentUser, backAction = null, screenType = "internal") {
  ...
  return `
        <div class="top-action-group">
                ...
                ` : ""}
                ${isMenu ? `
                <button class="fab-icon-btn fab-config" type="button" onclick="renderConfigSubMenu()" aria-label="Configura\xE7\xF5es" title="Configura\xE7\xF5es">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" width="36" height="36">
                        <circle cx="18" cy="18" r="18" fill="#EF2B2D"/>
                ...
                <button class="fab-icon-btn fab-desligar" type="button" onclick="logout()" aria-label="Desligar" title="Encerrar sess\xE3o">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" width="36" height="36">
...
function getOperationalIdentityHTML(type = "PICKING") {
  ...
  const label = isPick ? "SEPARA\xC7\xC3O (PICK)" : "CONFER\xCANCIA (PACK)";
  const icon = isPick ? "inventory_2" : "verified";
  const mobileGradient = isPick ? "linear-gradient(90deg, #EF4444 0%, #B91C1C 100%)" : "linear-gradient(90deg, #22C55E 0%, #15803D 100%)";
  return `
...
const MODULE_SIDEBAR_CONFIG = {
  ...
  kit_lampada: { label: "KIT L\xC2MPADAS", icon: "lightbulb", colorFrom: "#F59E0B", colorTo: "#B45309", shadow: "245,158,11" },
  movimentos: { label: "MOVIMENTOS", icon: "sync_alt", colorFrom: "#8B5CF6", colorTo: "#6D28D9", shadow: "139,92,246" },
  inventario: { label: "INVENT\xC1RIO", icon: "fact_check", colorFrom: "#F97316", colorTo: "#C2410C", shadow: "249,115,22" },
...
async function loadUsersOnly() {
  try {
    ...
    if (data && data.length > 0) {
      ...
      console.log(`[BOOT] usuarios -> Supabase (${appData.users.length} usu\xE1rios carregados)`);
    } else {
      console.log("[BOOT] usu\xE1rios: fallback para dados em mem\xF3ria ou fallback");
    }
  } catch (e) {
    ...
    try {
      ...
      if (data) {
        appData.users = data.filter((u) => String(u.ativo).toLowerCase() === "sim").map((u) => ({
          ...u,
          avatar_url: u.avatar_url && !["sim", "nao", "n\xE3o"].includes(String(u.avatar_url).toLowerCase()) && (String(u.avatar_url).startsWith("http") || String(u.avatar_url).startsWith("data:")) ? u.avatar_url : ""
        }));
      }
    } catch (e2) {
      console.warn("[BOOT] Tamb\xE9m falhou fetchSheetData:", e2);
    }
  }
}
let lastLoadAllDataCall = 0;
const LOAD_ALL_DATA_DEBOUNCE_MS = 3e3;
async function loadAllData(silent = false, caller = "unknown") {
  const now = Date.now();
  if (now - lastLoadAllDataCall < LOAD_ALL_DATA_DEBOUNCE_MS) {
    console.log(`[SYNC] loadAllData ignorado (debounce): ${caller} | \xDAltima: ${now - lastLoadAllDataCall}ms`);
  ...
  try {
    ...
    appData.lastSyncTime = (/* @__PURE__ */ new Date()).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
...
async function ensureCanaisLoaded(force = false) {
  ...
  try {
    ...
    if (window.supabaseClient) {
      const { data: directData, error } = await window.supabaseClient.from("canais_envio").select("*").eq("ativo", true).order("nome", { ascending: true });
      if (error) {
...
async function setUser(userName, userId, userProfile) {
  console.log("[LOGIN DEBUG] usu\xE1rio clicado");
  ...
  try {
    ...
    if (navigator.onLine) {
      ...
      console.log("[SYNC] setUser coordenando sincroniza\xE7\xE3o inicial...");
  ...
  if (costField && toggleIcon) {
    if (!isVisible) {
      ...
      if (activeEan) {
        ...
        console.warn("Contexto do produto perdido para exibi\xE7\xE3o do custo.");
      }
    } else {
...
function removeLegacyLoginFullscreenControls() {
  ...
  document.querySelectorAll(".top-bar-minimal, .btn-exit-minimal, .btn-exit-floating, #exit-fullscreen-float, #fullscreen-controls").forEach((el) => el.remove());
  document.querySelectorAll('button, [role="button"], div').forEach((el) => {
    ...
    const looksLikeClose = text === "close" || text === "x" || text === "\xC7";
    ...
    const isSmallFloatingControl = rect.width >= 30 && rect.width <= 90 && rect.height >= 30 && rect.height <= 90 && rect.top <= Math.max(180, window.innerHeight * 0.28);
    if (!isSmallFloatingControl) return;
    const style = window.getComputedStyle(el);
    const canBeOverlay = style.position === "fixed" || style.position === "absolute" || Number(style.zIndex) >= 1e3;
...
function renderLogin(push = true) {
  ...
  if (appData.users && appData.users.length > 0) {
    usersToRender = appData.users.map((u) => ({
  ...
  const userGridHTML = usersToRender.map((u, index) => {
    ...
    const formattedName = nameParts.length > 1 ? `${nameParts.slice(0, -1).join(" ")}<br>${nameParts[nameParts.length - 1]}` : u.nome || "";
    return `
    ...
    if (gridContainer) {
      ...
      if (!existingScreen.querySelector(".login-user-hint")) {
        existingScreen.insertAdjacentHTML("beforeend", `
                    <div class="login-user-hint" aria-label="Selecione seu usu\xE1rio">
            ...
            <main class="container">
                ...
                <div style="display: flex; flex-direction: column; gap: 24px;">
                    <div class="menu-card" onclick="renderEstoqueAtual()" style="cursor: pointer;">
                        ...
                        <div style="flex: 1;">
                            <span class="label" style="font-size: 16px; font-weight: 700;">Estoque Cr\xEDtico</span>
                            <span style="display: block; font-size: 0.85rem; color: var(--muted);">Produtos abaixo do estoque m\xEDnimo</span>
                        </div>
                    ...
                    <div class="menu-card" onclick="renderPickMenu()" style="cursor: pointer;">
                        ...
                        <div style="flex: 1;">
                            <span class="label" style="font-size: 16px; font-weight: 700;">Separa\xE7\xF5es Pendentes</span>
                            <span style="display: block; font-size: 0.85rem; color: var(--muted);">Filas de separa\xE7\xE3o aguardando</span>
                        </div>
                    ...
                    <div class="menu-card" style="opacity: 0.5; cursor: default;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <span style="display: block; font-size: 0.85rem; color: var(--muted);">Pedidos de compra em tr\xE2nsito</span>
                        </div>
...
const menuModulesConfig = [
  ...
  { id: "kit_lampada", label: "KIT L\xC2MPADAS", icon: "kit_lampada", order: 2, type: "principal" },
  { id: "pick", label: "SEPARA\xC7\xC3O (PICK)", icon: "pick", order: 3, type: "principal" },
  { id: "pack", label: "CONFER\xCANCIA (PACK)", icon: "pack", order: 4, type: "principal" },
  { id: "movimentacoes", label: "MOVIMENTOS", icon: "movimentacoes", order: 5, type: "principal" },
  { id: "inventario", label: "INVENT\xC1RIO", icon: "inventario", order: 6, type: "principal" },
...
function getMenuItemsFromConfig() {
  ...
  return menuModulesConfig.sort((a, b) => a.order - b.order).map((module) => {
...
function getQuickActionsHTML(modoRapidoAtivo) {
  return `
        ...
        <div id="quick-actions-menu" class="quick-actions-menu hidden" role="menu" aria-label="A\xE7\xF5es r\xE1pidas">
            <button class="quick-action-item" type="button" role="menuitem" onclick="quickActionNewMov()">
                ...
                <span>Movimenta\xE7\xE3o r\xE1pida</span>
            </button>
            <button class="quick-action-item" type="button" role="menuitem" onclick="quickActionEntrada()">
                <span class="material-symbols-rounded icon">login</span>
                <span>Entrada r\xE1pida</span>
            </button>
            <button class="quick-action-item" type="button" role="menuitem" onclick="quickActionSaida()">
                <span class="material-symbols-rounded icon">logout</span>
                <span>Sa\xEDda r\xE1pida</span>
            </button>
        ...
        <button class="quick-action-fab fab-icon-btn fab-funcoes ${modoRapidoAtivo ? "fast-mode" : ""}" type="button" onclick="${modoRapidoAtivo ? "showToast(&quot;Modo r\xE1pido ativo. Use Confer\xEAncia/Sa\xEDda direta.&quot;, &quot;info&quot;)" : "toggleQuickActions()"}" aria-label="Fun\xE7\xF5es r\xE1pidas" title="Fun\xE7\xF5es r\xE1pidas">
            <span id="quick-action-icon" class="material-symbols-rounded">add</span>
        </button>
    `;
}
  ...
  (appData.estoque || []).forEach((item) => {
    ...
    if (!consolidated[id]) {
      ...
      consolidated[id] = {
        ...
        descricao: product ? product.descricao_base || product.nome || product.col_b : item.descricao || item.col_b || "Sem Descri\xE7\xE3o",
        saldo_total: 0,
        saldo_disponivel: 0,
        saldo_reservado: 0,
        saldo_em_transito: 0,
                                    ...
                                    <div style="background: var(--surface); padding: 16px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05);">
                                        ...
                                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-bottom: 12px;">
                                            <div style="background: rgba(255,255,255,0.03); padding: 8px; border-radius: 8px; text-align: center;">
                                                <div style="font-size: 0.5rem; color: var(--muted); text-transform: uppercase;">Dispon\xEDvel</div>
                                            ...
                                            <div style="background: rgba(255,255,255,0.03); padding: 8px; border-radius: 8px; text-align: center;">
                                                <div style="font-size: 0.5rem; color: var(--muted); text-transform: uppercase;">Tr\xE2nsito</div>
...
function renderComprasSubMenu() {
  ...
  const subItems = [
    ...
    { label: "HIST\xD3RICO", icon: menu3DIcons.historico, action: "renderHistoricoComprasScreen()" }
  ];
  app.innerHTML = `
...
function renderFornecedoresScreen() {
  renderComprasShell("FORNECEDORES", "Cadastro preparado para futura integra\xE7\xE3o com Supabase.", `
        <section class="compras-panel fornecedores-panel">
            <div class="compras-toolbar">
                ...
                <button class="compras-primary-btn" type="button" onclick="showToast('Cadastro de fornecedor ser\xE1 integrado em uma pr\xF3xima fase.', 'info')">
            ...
            <div class="compras-empty-state">
                ...
                <p>Quando a integra\xE7\xE3o for criada, os fornecedores aparecer\xE3o nesta lista.</p>
            </div>
        </section>
    `);
}
  ...
  renderComprasShell("PEDIDO DE COMPRA", "Rascunho visual para montar pedidos de compra.", `
        <section class="compras-panel compras-order-panel">
            <div class="compras-form-grid">
                ...
                <button class="compras-secondary-btn" type="button" onclick="showToast('Itens do pedido ser\xE3o integrados em uma pr\xF3xima fase.', 'info')">
            ...
            <div class="compras-list-placeholder">
                ...
                <p>Os produtos adicionados ao pedido aparecer\xE3o aqui.</p>
            </div>
            <div class="compras-summary-row">
                <span>Total</span>
                <strong>R$ 0,00</strong>
                <button class="compras-primary-btn" type="button" onclick="showToast('Finaliza\xE7\xE3o visual preparada para fase futura.', 'info')">
...
function renderPedidosTransporteScreen() {
  renderComprasShell("PEDIDOS EM TRANSPORTE", "Pedidos comprados que ainda n\xE3o chegaram.", `
        <section class="compras-panel">
            <div class="compras-status-strip">
                <div>
                    <span>Em tr\xE2nsito</span>
                    <strong>0</strong>
                </div>
                <div>
                    <span>Atrasados</span>
                ...
                <div class="inv-initial-note">
                    ...
                    <div>
                        <strong>INVENT\xC1RIO INICIAL</strong>
                        <p>Como este \xE9 o primeiro invent\xE1rio, n\xE3o existe saldo anterior no sistema. Por isso, exibimos apenas o Saldo Inicial (quantidade contada).</p>
                    </div>
                </div>
                ` : ""}
            </div>
            <div class="compras-empty-state">
                ...
                <p>Futuramente, compras pendentes de recebimento ser\xE3o acompanhadas aqui.</p>
            </div>
        </section>
    `);
}
function renderHistoricoComprasScreen() {
  renderComprasShell("HIST\xD3RICO DE COMPRAS", "Consulta visual preparada para compras finalizadas.", `
        <section class="compras-panel">
            <div class="compras-toolbar compras-history-toolbar">
                <label class="compras-search-field">
                    <span class="material-symbols-rounded">search</span>
                    <input type="search" placeholder="Buscar compra, fornecedor ou NF..." aria-label="Buscar hist\xF3rico de compras">
                </label>
                <div class="compras-filter-row" aria-label="Filtros de hist\xF3rico">
            ...
            <div class="compras-list-placeholder compras-table-empty">
                ...
                <strong>Nenhum hist\xF3rico de compras</strong>
                <p>As compras registradas aparecer\xE3o nesta \xE1rea em formato de lista/tabela.</p>
            </div>
        </section>
    `);
}
...
function renderMovimentacoesSubMenu() {
  const currentUser = localStorage.getItem("currentUser");
  const subItems = [
    { id: "transferencia", label: "TRANSFER\xCANCIA", icon: "movimentacoes", action: "renderTransferenciaScreen()" },
    ...
    { id: "historico_mov", label: "HIST\xD3RICO", icon: "historico", action: "renderMovimentacoesHistory()" }
  ];
  app.innerHTML = `
        ...
        <div class="dashboard-screen internal fade-in" style="background: #232323; min-height: 100vh;">
            ...
            <main class="container" style="display: flex; align-items: center; justify-content: center; height: calc(100vh - 80px);">
                <div style="text-align: center; color: var(--muted);">
                    ...
                    <p>Formul\xE1rio de Envio para Defeito em desenvolvimento.</p>
                </div>
            </main>
        </div>
    `;
}
const STOCK_LOCALS = ["T\xC9RREO", "MOSTRU\xC1RIO", "1\xBA ANDAR", "DEFEITO", "EM GARANTIA", "EM TRANSPORTE"];
const MOVIMENTACAO_ORIGINS = [
  { value: "MANUAL", label: "Manual" },
  { value: "PEDIDO", label: "Pedido" },
  { value: "NOTA_FISCAL", label: "Nota Fiscal" },
  { value: "INVENTARIO", label: "Invent\xE1rio" },
  { value: "CONFERENCIA", label: "Confer\xEAncia" },
  { value: "SEPARACAO", label: "Separa\xE7\xE3o" }
];
...
function renderMovimentacoesList(history2) {
  ...
  const getTipoBadge = (tipo) => {
    const colors = {
      ...
      "SA\xCDDA": { bg: "rgba(239,68,68,0.15)", color: "#ef4444" },
...
function filterMovimentacoes() {
  ...
  if (search) {
    filtered = filtered.filter(
      (m) => (m.id_interno || "").toLowerCase().includes(search) || (m.descricao || "").toLowerCase().includes(search)
    );
  }
  if (tipo) {
...
function openNovaMovimentacaoModal() {
  console.log("[MOV] clique em Nova movimenta\xE7\xE3o");
        ...
        <div id="nova-mov-modal" class="modal-overlay" onclick="closeNovaMovimentacaoModal(event)">
            <div class="modal-content" onclick="event.stopPropagation()">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                    <h2 style="font-size: 1.1rem; font-weight: 700; margin: 0;">Nova Movimenta\xE7\xE3o</h2>
                ...
                <div class="input-group" style="margin-bottom: 20px;">
                    <label style="font-size: 0.8rem; font-weight: 600; color: var(--muted); margin-bottom: 8px; display: block;">Tipo de Movimenta\xE7\xE3o</label>
                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px;">
                        ${["ENTRADA", "SAIDA", "TRANSFERENCIA", "AJUSTE"].map((tipo) => `
                            <button type="button" class="tipo-mov-btn" data-tipo="${tipo}" onclick="selectTipoMovimentacao('${tipo}', this)" style="padding: 12px 8px; border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; background: rgba(255,255,255,0.02); color: white; cursor: pointer; font-size: 0.75rem; font-weight: 600; transition: all 0.2s;">
                                ${tipo === "ENTRADA" ? "Entrada" : tipo === "SAIDA" ? "Sa\xEDda" : tipo === "TRANSFERENCIA" ? "Transfer\xEAncia" : "Ajuste"}
                            </button>
                        `).join("")}
                    </div>
                </div>
                ...
                <div id="mov-form-container">
                    <div style="text-align: center; padding: 40px 20px; color: var(--muted);">
                        ...
                        <p style="font-size: 0.85rem;">Selecione o tipo de movimenta\xE7\xE3o acima</p>
                    </div>
                </div>
                ${false ? `
                <div class="inv-initial-note">
...
function closeNovaMovimentacaoModal(event2) {
  if (event2 && event2.target !== event2.currentTarget && !event2.target.closest(".modal-content")) return;
  const modal = document.getElementById("nova-mov-modal");
  if (modal) {
    modal.style.opacity = "0";
...
function renderMovimentacaoForm(tipo) {
  ...
  const getMotivoField = () => `
        <div class="input-group full-width">
            <label>Motivo</label>
            <input type="text" id="mov-motivo" class="input-field" placeholder="Ex: Contagem invent\xE1rio, ajuste sistema...">
        </div>
    `;
  let fieldsHTML = "";
  let submitLabel = "";
  ...
  switch (tipo) {
    ...
    case "SAIDA":
      ...
      submitLabel = "Registrar Sa\xEDda";
    ...
    case "TRANSFERENCIA":
      ...
      submitLabel = "Confirmar Transfer\xEAncia";
...
function searchProductForMovInModal() {
  ...
  const results = (appData.products || []).filter(
    (p) => (p.descricao_base || "").toLowerCase().includes(query) || (p.ean || "").toString().toLowerCase().includes(query) || (p.id_interno || "").toString().toLowerCase().includes(query)
  ).slice(0, 5);
...
async function saveNovaMovimentacao(tipo) {
  ...
  if (!selectedProductForMov && inputValue) {
    ...
    const produtoEncontrado = produtosDisponiveis.find((p) => {
      ...
      const ean = p.ean || "";
      return idInterno.toString().trim() === inputValue || idInterno.toString().includes(inputValue) || ean.toString().trim() === inputValue;
    });
    if (produtoEncontrado) {
      ...
      console.log("[MOV] n\xE3o achou em appData, tentando DataClient...");
      try {
        ...
        if (data && data.products && data.products.length > 0) {
          ...
          const produtoDataClient = data.products.find((p) => {
            const idInterno = p.id_interno || p.col_a || "";
            return idInterno.toString().trim() === inputValue || idInterno.toString().includes(inputValue);
          });
  ...
  if (!selectedProductForMov) {
    ...
    console.log("[MOV] razao: selectedProductForMov \xE9 null");
    showToast("Produto n\xE3o encontrado. Selecione da lista ou digite ID correto.");
    isFinalizing = false;
    console.log("=========================================");
    return;
  }
  ...
  if (!qtyInput) {
    ...
    showToast("Erro: campo de quantidade n\xE3o encontrado.");
    isFinalizing = false;
    console.log("=========================================");
    return;
  }
  ...
  if (isNaN(qty) || qty <= 0) {
    ...
    showToast("Quantidade inv\xE1lida. Digite um n\xFAmero maior que 0.");
    isFinalizing = false;
    console.log("=========================================");
    return;
  }
  ...
  try {
    ...
    if (stockSuccess) {
      console.log("[MOV] fluxo conclu\xEDdo com sucesso");
      console.log("[MOV] resposta de sucesso");
      showToast("Movimenta\xE7\xE3o registrada com sucesso!");
      ...
      console.log("[MOV] modal fechado ap\xF3s sucesso");
      ...
      console.log("[MOV] ERRO: movimento salvo mas estoque n\xE3o atualizado");
      console.log("[MOV] resposta de erro");
      showToast("Erro: Movimento salvo, mas estoque n\xE3o atualizado.");
...
function renderTransferenciaForm() {
  const currentUser = localStorage.getItem("currentUser");
  const locals = ["TERREO", "1\xBAANDAR", "MOSTRUARIO", "DEFEITO"];
  app.innerHTML = `
                <div class="dashboard-screen fade-in internal">
                    ${getTopBarHTML(currentUser, "renderMovimentacoesSubMenu()")}
                    <main class="container">
                        <div class="sub-menu-header">
                            <h2 style="font-size: 1.2rem; font-weight: 700;">TRANSFER\xCANCIA</h2>
                        </div>
                        <div class="form-grid" style="background: var(--surface); padding: 24px; border-radius: 24px; border: 1px solid rgba(255,255,255,0.05);">
                            <div class="input-group full-width">
                                <label>Produto (EAN ou ID)</label>
                            ...
                            <div style="display: flex; gap: 16px; margin-top: 24px; width: 100%;">
                                ...
                                <button class="btn-action" style="flex: 2; justify-content: center;" onclick="saveMovimentacao('TRANSFER\xCANCIA')">Confirmar</button>
                            </div>
                        </div>
                    </main>
                </div>
                ...
                <div class="dashboard-screen fade-in internal">
                    ...
                    <main class="container">
                        ...
                        <div class="form-grid" style="background: var(--surface); padding: 24px; border-radius: 24px; border: 1px solid rgba(255,255,255,0.05);">
                            ...
                            <div class="input-group full-width">
                                <label>Motivo / Observa\xE7\xE3o</label>
...
function searchProductForMov() {
  ...
  const results = appData.products.filter(
    (p) => (p.descricao_base || "").toLowerCase().includes(query) || (p.ean || "").toString().includes(query) || (p.id_interno || "").toString().includes(query)
  ).slice(0, 5);
  resultsDiv.innerHTML = results.map((p) => `
...
async function saveMovimentacao(tipo) {
  ...
  if (!qtyInput || !origemInput) {
    showToast("Erro: Campos de movimenta\xE7\xE3o n\xE3o encontrados.");
    return;
  }
  ...
  const movData = {
    tipo: tipo === "TRANSFER\xCANCIA" ? "TRANSFERENCIA" : tipo,
    id_interno: selectedProductForMov.id_interno || selectedProductForMov.col_a,
    local_origem: localOrigem,
    local_destino: localDestino,
    quantidade: qty,
  ...
  try {
    ...
    if (movData.tipo === "ENTRADA") {
      stockSuccess = await DataClient.updateEstoqueSupabase(idInterno, movData.local_destino, "soma", movData.quantidade);
    } else if (movData.tipo === "SA\xCDDA" || movData.tipo === "SAIDA") {
      stockSuccess = await DataClient.updateEstoqueSupabase(idInterno, movData.local_origem, "subtrai", movData.quantidade);
    ...
    if (stockSuccess) {
      ...
      console.error("[Supabase] Falha Cr\xEDtica: Movimento gravado, mas estoque N\xC3O atualizado.");
      showToast("Erro: Movimento gravado, mas o saldo n\xE3o p\xF4de ser atualizado.");
    }
  } catch (e) {
...
async function renderMovimentacoesHistory() {
  ...
  try {
    ...
    const history2 = await DataClient.fetchMovimentosSupabase();
    console.log("[MOVIMENTOS DEBUG] movimentos carregados:", history2?.length || 0);
    const listContainer = document.getElementById("movimentacoes-list-container");
    if (listContainer) {
      if (!history2 || history2.length === 0) {
        console.log("[MOVIMENTOS DEBUG] Nenhuma movimenta\xE7\xE3o encontrada no banco.");
        listContainer.innerHTML = `<div style="text-align: center; padding: 40px; color: var(--muted);">Nenhum movimento encontrado</div>`;
      } else {
        listContainer.innerHTML = renderMovimentacoesList(history2);
        listContainer.style.textAlign = "left";
        listContainer.style.padding = "0";
      }
    }
  } catch (err) {
    console.error("[MOVIMENTOS DEBUG] erro cr\xEDtico ao processar movimentos:", err);
    const listContainer = document.getElementById("movimentacoes-list-container");
    if (listContainer) {
      listContainer.innerHTML = `<div style="text-align: center; padding: 40px; color: var(--muted);">Nenhum movimento encontrado</div>`;
    }
...
function normalizeLocal(local) {
  if (!local) return "";
  let norm = local.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(/\s+/g, "_").replace("1\xBA_ANDAR", "PRIMEIRO_ANDAR").replace("1\xBA_ANDAR", "PRIMEIRO_ANDAR").replace("1_ANDAR", "PRIMEIRO_ANDAR");
...
function prettyLocal(local) {
  ...
  const map = {
    "TERREO": "T\xC9RREO",
    "MOSTRUARIO": "MOSTRU\xC1RIO",
    "PRIMEIRO_ANDAR": "1\xBA ANDAR",
    "DEFEITO": "DEFEITO",
...
async function renderTransferenciaScreen() {
  ...
  console.log("[INV-DIAG] Garantindo produtos carregados na Transfer\xEAncia...");
  ...
  const locals = ["T\xC9RREO", "MOSTRU\xC1RIO", "1\xBA ANDAR", "DEFEITO", "EM GARANTIA", "EM TRANSPORTE", "FULL ML"];
  app.innerHTML = `
        <div class="dashboard-screen internal fade-in" style="background: #232323; min-height: 100vh;">
            ${getTopBarHTML(currentUser, "renderMovimentacoesSubMenu()")}
            <div class="transfer-screen" style="padding: 20px; color: white; max-width: 600px; margin: 0 auto;">
                ...
                <div style="position: fixed; bottom: 0; left: 0; width: 100%; padding: 20px; background: #232323; border-top: 1px solid #333; z-index: 100;">
                    <div style="max-width: 600px; margin: 0 auto;">
                        <button id="btn-confirm-transfer" onclick="confirmTransferencia()" style="width: 100%; padding: 20px; border-radius: 16px; background: #4ade80; color: #111; font-weight: 800; font-size: 1.1rem; border: none; cursor: pointer;">CONFIRMAR TRANSFER\xCANCIA</button>
                    </div>
                </div>
            </div>
        </div>
...
async function addTransferItem() {
  ...
  if (!product) {
    console.log("[INV-DIAG] produto n\xE3o no cache, tentando recarga for\xE7ada...");
    await ensureProdutosLoaded(true);
    product = appData.products.find((p) => p.ean == ean || p.id_interno == ean);
  }
  console.log("[INV-DIAG] produto encontrado:", product);
  if (!product) {
    showToast("Produto n\xE3o encontrado!", "error");
    eanInput.value = "";
    return;
  }
  const idInterno = product.id_interno;
  const origemNorm = normalizeLocal(origem);
  const { data: stockData, error } = await window.supabaseClient.from("estoque_atual").select("saldo_disponivel").eq("id_interno", idInterno).eq("local", origemNorm).maybeSingle();
  if (error) {
...
async function adjustTransferItemQty(index, delta) {
  ...
  if (delta > 0) {
    const origemNorm = normalizeLocal(origem);
    const { data: stockData, error } = await window.supabaseClient.from("estoque_atual").select("saldo_disponivel").eq("id_interno", item.id_interno).eq("local", origemNorm).maybeSingle();
...
async function confirmTransferencia() {
  ...
  showToast("Iniciando transfer\xEAncia...");
  ...
  try {
    ...
    for (const item of items) {
      ...
      const { data: estoqueOrigem, error: fetchOrigemErr } = await client.from("estoque_atual").select("*").eq("id_interno", item.id_interno).eq("local", origemNorm).maybeSingle();
      if (fetchOrigemErr) {
      ...
      const { data: currentDestino, error: fetchDestinoErr } = await client.from("estoque_atual").select("*").eq("id_interno", item.id_interno).eq("local", destinoNorm).maybeSingle();
      if (fetchDestinoErr) {
      ...
      const movPayload = {
        ...
        observacao: "Transfer\xEAncia manual",
        usuario: localStorage.getItem("currentUser")
      };
      console.log("[TRANSF-DIAG] valida\xE7\xE3o ok:", item.id_interno);
      ...
      const { data: resultOrigem, error: updateOrigemErr } = await client.from("estoque_atual").update(payloadOrigem).eq("id_interno", item.id_interno).eq("local", origemNorm).select();
      if (updateOrigemErr) {
      ...
      if (currentDestino) {
        ...
        const { data, error } = await client.from("estoque_atual").update(payloadDestino).eq("id_interno", item.id_interno).eq("local", destinoNorm).select();
        if (error) {
        ...
        const { data, error } = await client.from("estoque_atual").insert([payloadDestino]).select();
        if (error) {
      ...
      const { data: resultMov, error: movErr } = await client.from("movimentos").insert([movPayload]).select();
      if (movErr) {
        ...
        <div class="dashboard-screen internal fade-in" style="background: #101018; min-height: 100vh;">
            ...
            <main class="container" style="max-width: 560px; margin: 0 auto; padding: 20px;">
                <div style="text-align: center; margin-bottom: 32px;">
                    ...
                    <p style="color: var(--muted); font-size: 0.85rem;">Corre\xE7\xE3o manual de saldo por produto e local</p>
                </div>
                ...
                <form id="ajuste-form" onsubmit="event.preventDefault(); saveAjusteEstoque()" style="display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <div class="input-group">
                        ...
                        <select id="ajuste-local" class="op-input" onchange="updateAjusteStockInfo()">
                            <option value="T\xC9RREO">T\xC9RREO</option>
                            <option value="MOSTRU\xC1RIO">MOSTRU\xC1RIO</option>
                            <option value="1\xBA ANDAR">1\xBA ANDAR</option>
                    ...
                    <div class="input-group">
                        ...
                        <select id="ajuste-motivo" class="op-input">
                            <option value="correcao_contagem">Corre\xE7\xE3o de contagem</option>
                            <option value="avaria">Avaria</option>
                            <option value="perda">Perda</option>
                            <option value="devolucao">Devolu\xE7\xE3o</option>
                            <option value="garantia">Garantia</option>
                            <option value="erro_lancamento">Erro de lan\xE7amento</option>
                    ...
                    <!-- 6. Observa\xE7\xE3o -->
                    <div class="input-group full-width">
                        <label style="color: #888; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 700; margin-bottom: 8px; display: block;">Observa\xE7\xE3o</label>
...
function searchProductForAjuste() {
  ...
  const results = (appData.products || []).filter(
    (p) => (p.descricao_base || "").toLowerCase().includes(query) || (p.descricao_completa || "").toLowerCase().includes(query) || (p.ean || "").toString().toLowerCase().includes(query) || (p.id_interno || "").toString().toLowerCase().includes(query)
  ).slice(0, 6);
  resultsDiv.innerHTML = results.map((p) => `
...
async function updateAjusteStockInfo() {
  if (!ajusteSelectedProduct) return;
  const localRaw = document.getElementById("ajuste-local")?.value || "T\xC9RREO";
...
async function saveAjusteEstoque() {
  ...
  try {
    ...
    const qty = parseFloat(qtyRaw);
    if (isNaN(qty) || qty < 0) {
      showToast("Quantidade inv\xE1lida. Digite um n\xFAmero >= 0.", "error");
      isFinalizing = false;
      return;
    }
    ...
    if (novoSaldo < 0) {
      ...
      if (!config.permitir_saida_estoque_zero) {
        showToast('Estoque ficaria negativo. Habilite "Permitir sa\xEDda com estoque zerado" nas Configura\xE7\xF5es.', "error");
        isFinalizing = false;
        return;
      }
    }
    const motivoLabels = {
      "correcao_contagem": "Corre\xE7\xE3o de contagem",
      "avaria": "Avaria",
      "perda": "Perda",
      "devolucao": "Devolu\xE7\xE3o",
      "garantia": "Garantia",
    ...
    if (stockSuccess) {
      ...
      showToast("Movimento salvo, mas estoque N\xC3O foi atualizado.", "error");
    }
  } catch (e) {
  ...
  app.innerHTML = `
        <div class="dashboard-screen internal fade-in" style="background: #232323; min-height: 100vh;">
            ...
            <div id="inventory-setup-content" style="padding: 20px; display: flex; flex-direction: column; align-items: center; justify-content: center; height: calc(100vh - 80px);">
                ...
                <p style="color: #aaa; margin-top: 15px;">Validando sess\xF5es no servidor...</p>
            </div>
        </div>
    `;
  try {
    ...
    console.log("[INV-DIAG] Invent\xE1rios no hist\xF3rico local:", appData.inventario?.length || 0);
    ...
    if (data && data.inventario) {
      ...
      console.log("[INV-DIAG] Invent\xE1rios carregados do Supabase:", appData.inventario.length);
    }
    ...
    console.log("[INV-DIAG] Sessao aberta encontrada?", aberto ? `SIM (${aberto.inventario_id})` : "N\xC3O");
    ...
    if (aberto) {
      ...
      content.innerHTML = `
                <div style="text-align: center; max-width: 400px; width: 100%;">
                    ...
                    <h2 style="color: white; margin-bottom: 10px;">Invent\xE1rio Aberto Detectado</h2>
                    <p style="color: #aaa; margin-bottom: 30px;">J\xE1 existe uma sess\xE3o de invent\xE1rio <b>${type.toUpperCase()}</b> aberta no local <b>${local}</b>.</p>
                    ...
                    <div style="background: rgba(255,255,255,0.05); padding: 20px; border-radius: 16px; margin-bottom: 30px; text-align: left; border: 1px solid rgba(255,255,255,0.1);">
                        ...
                        <div style="font-size: 0.7rem; color: #888; text-transform: uppercase; font-weight: 700;">Data de In\xEDcio</div>
    ...
    console.error(e);
    showToast("Erro de conex\xE3o", "error");
...
function renderInventoryStartCard(type) {
  ...
  const locais = [
    { value: "TT\xC9RREO", label: "TERREO", icon: "apartment" },
    { value: "MOSTRU\xC1RIO", label: "MOSTRUARIO", icon: "storefront" },
    { value: "1\xEF\xBD\xBA ANDAR", label: "1\xEF\xBD\xBA ANDAR", icon: "layers" },
...
async function createNewInventorySession(type) {
  ...
  const localRaw = document.getElementById("setup-local")?.value || "T\xC9RREO";
  ...
  try {
    ...
    console.log("[INV-DIAG] Criando nova sess\xE3o:", sessionId);
    ...
    const client = window.supabaseClient;
    if (!client) {
      alert("Erro: Supabase n\xE3o conectado");
      return;
    }
    ...
    showToast("Erro ao iniciar sess\xE3o", "error");
  } finally {
...
async function resumeInventorySession(sessionId, type) {
  try {
    ...
    const { data: invData, error: headerErr } = await client.from("inventarios").select("*").eq("inventario_id", sessionId).maybeSingle();
    if (headerErr) {
      console.error("[INV-DIAG] erro header:", headerErr);
      showToast("Erro ao carregar cabe\xE7alho");
      return;
    }
    if (!invData) {
      console.error("[INV-DIAG] invent\xE1rio n\xE3o encontrado no banco:", sessionId);
      showToast("Sessao n\xE3o encontrada no servidor");
      return;
    }
    console.log("[INV-DIAG] buscando itens para inventario:", sessionId);
    const { data: itens, error: itensErr } = await client.from("inventarios_itens").select("*").eq("inventario_id", sessionId);
    ...
    appData.currentInventory = {
      ...
      date: invData.data_inicio || (/* @__PURE__ */ new Date()).toISOString(),
      local: invData.local || "T\xC9RREO",
    ...
    appData.currentInventory.items = (itens || []).map((i) => {
      ...
      return {
        ...
        name: product?.descricao_completa || product?.nome || `PRODUTO ID: ${i.id_interno} (N\xC3O CARREGADO)`,
        brand: product?.marca || ""
      };
    });
    console.log("[INV-DIAG] currentInventory.items ap\xF3s map:", appData.currentInventory.items.length);
    await renderInventarioInicialScreen(sessionId);
  } catch (e) {
    console.error("[INV-DIAG] Erro cr\xEDtico ao retomar:", e);
    showToast("Erro t\xE9cnico ao retomar", "error");
  }
}
async function confirmCancelInventory(sessionId) {
  if (confirm("ATEN\xC7\xC3O: Deseja realmente CANCELAR este invent\xE1rio?")) {
    try {
      const client = window.supabaseClient;
      if (client) {
        await client.from("inventarios").update({
          status: "CANCELADO",
          data_fim: (/* @__PURE__ */ new Date()).toISOString(),
          atualizado_em: (/* @__PURE__ */ new Date()).toISOString()
  ...
  if (!appData.products || appData.products.length === 0) {
    ...
    app.innerHTML = `
            <div class="dashboard-screen internal fade-in" style="background: #232323; min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; color: white; text-align: center;">
                ...
                <div style="font-weight: 800; font-size: 1.2rem;">Sincronizando Produtos...</div>
                <div style="color: #777; font-size: 0.9rem; margin-top: 8px;">Isso levar\xE1 apenas alguns segundos.</div>
            </div>
        `;
    await ensureProdutosLoaded(true);
  } else {
                    ...
                    <div class="inv-footer-inner">
                        ${isView ? `
                            <div style="width:100%; display:flex; align-items:center; justify-content:space-between; gap:12px;">
                                ...
                                ` : `
                                    <button disabled class="inv-btn-status">SESS\xC3O ${inv.status}</button>
                                `}
                            </div>
                            ` : `
                            <span class="inv-total-label">Total de itens: <span id="inv-total-count" class="inv-total-num">0</span></span>
...
async function gerarPdfPreInventario() {
  ...
  if (!inv || !inv.items || inv.items.length === 0) {
    showToast("Nenhum item bipado para gerar o relat\xF3rio.", "warning");
    return;
  }
  try {
    ...
    const currentUser = localStorage.getItem("currentUser") || "N\xE3o identificado";
    doc.setFontSize(16);
    doc.text("RELAT\xD3RIO PR\xC9-FINALIZA\xC7\xC3O DO INVENT\xC1RIO", 105, 15, { align: "center" });
    doc.setFontSize(10);
    doc.text(`ID Invent\xE1rio: ${inv.id}`, 14, 25);
    doc.text(`Local: ${inv.local}`, 14, 30);
    doc.text(`Tipo: ${(inv.tipo || inv.type || "").toUpperCase()}`, 14, 35);
    doc.text(`Gerado em: ${timestamp}`, 14, 40);
    doc.text(`Respons\xE1vel: ${currentUser}`, 14, 45);
    ...
    doc.autoTable({
      startY: 70,
      head: [["ID Interno", "SKU", "EAN", "Descri\xE7\xE3o", "Marca", "Qtd Bipada"]],
      body: tableData,
      headStyles: { fillColor: [227, 6, 19] },
      // Cor primária do app #E30613
      theme: "grid",
      styles: { fontSize: 7 },
      // Reduzi um pouco a fonte para caber a nova coluna
      margin: { top: 70 }
    });
...
async function handleInventoryCamera(input) {
  ...
  showToast("Captura de c\xE2mera detectada. Processando...", "info");
  setTimeout(() => {
    showToast("Leitura via c\xE2mera em fase de implementa\xE7\xE3o t\xE9cnica.", "warning");
    input.value = "";
  }, 1500);
}
async function addInventoryItem(scannedEan = null) {
  ...
  console.log("[INV-DIAG] produto encontrado:", product ? `${product.id_interno} - ${product.descricao_completa}` : "N\xC3O");
  if (!product) {
    console.log(`[INV-DIAG] Produto ${ean} n\xE3o encontrado no cache. Tentando recarregar banco...`);
    await ensureProdutosLoaded(true);
    product = appData.products.find((p) => p.ean?.toString() === ean || p.id_interno?.toString() === ean || p.sku_fornecedor?.toString() === ean);
  }
  if (!product) {
    console.warn("[INV-DIAG] Produto n\xE3o encontrado em definitivo:", ean);
    playBeep(false);
    showToast("PRODUTO N\xC3O ENCONTRADO!", "error");
    if (eanInput) eanInput.value = "";
    return;
  }
...
async function saveInventoryItemToServer(item) {
  const client = window.supabaseClient;
  if (!client) {
    console.error("[INV-DIAG] Supabase client n\xE3o encontrado");
    return;
  }
  const inv = appData.currentInventory;
  if (!inv || !inv.id) {
    console.error("[INV-DIAG] Sessao de invent\xE1rio inv\xE1lida");
    return;
  }
  if (inv.isNewSession) {
    console.log("[INVENTARIO DEBUG] primeiro item bipado, criando invent\xE1rio");
    const payload2 = {
    ...
    const { error } = await client.from("inventarios").insert([payload2]);
    if (error) {
      console.error("[INVENTARIO DEBUG] Erro ao criar invent\xE1rio no Supabase:", error);
      showToast("Erro ao criar sess\xE3o de invent\xE1rio.", "error");
      return;
    }
    inv.isNewSession = false;
    if (!appData.inventario) appData.inventario = [];
    appData.inventario.unshift(payload2);
  }
  ...
  try {
    const { data: existing, error: selectErr } = await client.from("inventarios_itens").select("id_interno").eq("inventario_id", inv.id).eq("id_interno", item.id_interno).maybeSingle();
    if (selectErr) console.error("[INV-DIAG] erro ao verificar exist\xEAncia:", selectErr);
    let result;
    if (existing) {
      console.log("[INV-DIAG] item j\xE1 existe, executando UPDATE...");
      result = await client.from("inventarios_itens").update(payload).eq("inventario_id", inv.id).eq("id_interno", item.id_interno);
    } else {
      console.log("[INV-DIAG] item novo, executando INSERT...");
      result = await client.from("inventarios_itens").insert([payload]);
    }
    if (result.error) {
      ...
      console.log("[INV-DIAG] Persist\xEAncia OK. Iniciando confirma\xE7\xE3o imediata...");
      const { data: check, error: checkError } = await client.from("inventarios_itens").select("*").eq("inventario_id", inv.id).eq("id_interno", item.id_interno);
      console.log("[INV-DIAG] confirma\xE7\xE3o inventarios_itens:", check, checkError);
      if (!check || check.length === 0) {
        console.error("[INV-DIAG] FALHA CR\xC1GICA: Item n\xE3o encontrado ap\xF3s salvamento!");
        showToast("Item N\xC3O foi salvo no banco!", "error");
        throw new Error("Persist\xEAncia falhou");
      } else {
        console.log("[INV-DIAG] Confirma\xE7\xE3o FINALIZADA COM SUCESSO.");
      }
    }
  } catch (e) {
...
window.finishInventorySession = async function() {
  ...
  if (!appData.currentInventory?.items?.length) {
    showToast("N\xE3o \xE9 poss\xEDvel fechar um invent\xE1rio vazio!", "error");
    return;
  }
  isFinalizing = true;
  const client = window.supabaseClient;
  if (!client) {
    showToast("Supabase n\xE3o dispon\xEDvel", "error");
    isFinalizing = false;
    return;
  }
  ...
  try {
    ...
    const { data: dbItems, error: fetchErr } = await client.from("inventarios_itens").select("*").eq("inventario_id", sessionId);
    if (fetchErr || !dbItems || dbItems.length === 0) {
      console.error("[INV-DIAG] erro ou nenhum item encontrado:", fetchErr);
      showToast("Nenhum item salvo no banco para este invent\xE1rio!", "error");
      isFinalizing = false;
      if (btn) {
        btn.disabled = false;
        btn.style.opacity = "1";
        btn.innerHTML = "FINALIZAR INVENT\xC1RIO";
      }
      return;
    }
    ...
    for (const item of dbItems) {
      ...
      if (diferenca !== 0) {
        ...
        else movTipo = diferenca > 0 ? "AJUSTE+" : "AJUSTE-";
        const movPayload = {
          tipo: movTipo,
          id_interno: item.id_interno,
          local_origem: null,
    ...
    showToast("Invent\xE1rio finalizado com sucesso!");
    appData.currentInventory = null;
    renderInventorySuccessScreen();
  } catch (err) {
    console.error("[INV-DIAG] ERRO CR\xC1GICO na finaliza\xE7\xE3o:", err);
  ...
  app.innerHTML = `
                <div class="dashboard-screen fade-in" style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100vh; text-align: center; padding: 20px;">
                    <div style="background: var(--surface); padding: 40px; border-radius: 24px; border: 1px solid var(--primary); box-shadow: 0 20px 40px rgba(0,0,0,0.4); max-width: 400px; width: 100%;">
                        ...
                        <h2 style="font-size: 1.8rem; margin-bottom: 10px; color: white;">INVENT\xC1RIO SALVO!</h2>
...
function renderInventarioSubMenu() {
  ...
  if (appData.currentInventory && appData.currentInventory.isNewSession) {
    console.log("[INVENTARIO DEBUG] usu\xE1rio saiu sem itens, nada salvo");
    appData.currentInventory = null;
  }
  const currentUser = localStorage.getItem("currentUser");
  const subItems = [
    { id: "inv_inicial", label: "INVENT\xC1RIO INICIAL", icon: "inventario_inicial", onclick: "startInventarioInicial()" },
    { id: "inv_geral", label: "INVENT\xC1RIO GERAL", icon: "inventario_geral", onclick: "startInventarioGeral()" },
    { id: "inv_parcial", label: "INVENT\xC1RIO PARCIAL", icon: "inventario_parcial", onclick: "renderInventorySetup('parcial')" },
    { id: "historico_inv", label: "HIST\xD3RICO", icon: "historico", onclick: "renderInventarioHistory()" }
  ];
  app.innerHTML = `
...
async function renderInventarioHistory() {
  ...
  try {
    ...
    console.log("[INV-DIAG] hist\xF3rico inventarios encontrados:", appData.inventario.length);
    const history2 = appData.inventario;
    app.innerHTML = `
            <div class="dashboard-screen internal fade-in" style="background: #232323; min-height: 100vh;">
                ${getTopBarHTML(currentUser, "renderInventarioSubMenu()")}
                <div style="padding: 20px; width: min(92vw, 860px); max-width: 860px; margin: 0 auto;">
                    <div style="display: flex; flex-direction: column; gap: 16px; margin-top: 10px;">
                        ${history2.length === 0 ? '<p style="color: #555; text-align: center; padding: 40px;">Nenhum registro encontrado.</p>' : history2.map((inv) => `
    ...
    console.error("[INV-DIAG] Erro ao carregar hist\xF3rico:", e);
    showToast("Erro ao carregar hist\xF3rico", "error");
  }
}
async function deleteTestInventory(sessionId) {
  if (!confirm(`EXCLUIR TESTE: Deseja apagar permanentemente todos os dados do invent\xE1rio ${sessionId}?
Isso apagar\xE1 itens, movimentos e o registro da sess\xE3o.`)) return;
  try {
    const client = window.supabaseClient;
    if (!client) return;
    showToast("Excluindo dados de teste...");
    await client.from("inventarios_itens").delete().eq("inventario_id", sessionId);
    await client.from("movimentos").delete().or(`observacao.ilike.%${sessionId}%,observacao.ilike.%Invent\xE1rio ${sessionId}%`);
    await client.from("inventarios").delete().eq("inventario_id", sessionId);
    showToast("Dados de teste exclu\xEDdos!", "success");
...
async function viewClosedInventory(sessionId) {
  try {
    ...
    showToast("Carregando invent\xE1rio fechado...");
    const client = window.supabaseClient;
    if (!client) {
      console.error("[INV-DIAG] Supabase Client n\xE3o dispon\xEDvel");
      return;
    }
    await ensureProdutosLoaded();
    const { data: invData, error: headerErr } = await client.from("inventarios").select("*").eq("inventario_id", sessionId).maybeSingle();
    console.log("[INV-DIAG] header encontrado:", invData ? "SIM" : "N\xC3O");
    if (headerErr) console.error("[INV-DIAG] erro header:", headerErr);
    const { data: itensData, error: itensErr } = await client.from("inventarios_itens").select("*").eq("inventario_id", sessionId);
    ...
    appData.currentInventory = {
      ...
      local: invData?.local || invData?.filtro_aplicado || "T\xC9RREO",
    ...
    console.log("[INV-DIAG] currentInventory.items ap\xF3s map (view):", appData.currentInventory.items.length);
    await renderInventarioInicialScreen(sessionId, "view");
  } catch (e) {
    console.error("[INV-DIAG] Erro cr\xEDtico ao visualizar:", e);
    showToast("Falha t\xE9cnica ao abrir visualiza\xE7\xE3o", "error");
  }
}
async function startInventoryReview(baseInventoryId) {
  ...
  if (!original || !original.items || original.items.length === 0) {
    alert("N\xE3o \xE9 poss\xEDvel revisar um invent\xE1rio sem itens contados!");
    return;
  }
  if (!confirm("Deseja iniciar uma REVIS\xC3O deste invent\xE1rio?\nSer\xE1 gerada uma nova sess\xE3o com os mesmos itens.")) return;
  try {
    showToast("Iniciando revis\xE3o...");
    const client = window.supabaseClient;
    if (!client) return;
    const currentUser = localStorage.getItem("currentUser");
    const date = /* @__PURE__ */ new Date();
    ...
    const { error: invErr } = await client.from("inventarios").insert([{
      ...
      filtro_aplicado: `REVIS\xC3O DO ${baseInventoryId}`
    }]);
    ...
    showToast("Revis\xE3o iniciada!", "success");
  } catch (e) {
    console.error("[INV] Erro ao criar revis\xE3o:", e);
    showToast("Erro ao criar revis\xE3o", "error");
  }
}
async function cancelClosedInventory(sessionId) {
  if (!confirm("Tem certeza que deseja ANULAR este invent\xE1rio?\n\nEssa a\xE7\xE3o N\xC3O apaga dados, apenas marca como ANULADO.")) return;
  try {
    showToast("Anulando invent\xE1rio...");
    const client = window.supabaseClient;
    if (!client) return;
    ...
    if (error) throw error;
    showToast("Invent\xE1rio anulado com sucesso!", "success");
    appData.currentInventory = null;
    ...
    console.error("[INV] Erro ao anular invent\xE1rio:", e);
    showToast("Erro ao anular invent\xE1rio", "error");
  }
}
async function renderStockCritical() {
  ...
  app.innerHTML = `
                <div class="dashboard-screen fade-in internal">
                    ...
                    <main class="container">
                        <div class="sub-menu-header">
                            <h2 style="font-size: 1.2rem; font-weight: 700;">ESTOQUE CR\xC1GICO</h2>
                        </div>
                        ...
                        <div id="critical-results">
                            ${criticalProducts.length === 0 ? `
                                <div style="text-align: center; padding: 40px; background: var(--surface); border-radius: 20px; color: var(--muted);">
                                    <span class="material-symbols-rounded" style="font-size: 48px; margin-bottom: 16px; color: #22c55e;">check_circle</span>
                                    <p>Nenhum produto com estoque cr\xEDtico.</p>
                                </div>
                            ` : `
                                <div style="display: flex; flex-direction: column; gap: 16px;">
                                    <p style="font-size: 0.8rem; font-weight: 700; color: var(--muted); text-transform: uppercase;">Produtos abaixo do m\xEDnimo (${criticalProducts.length})</p>
                                    ${criticalProducts.map((p) => `
                                        <div class="menu-card" style="flex-direction: row; justify-content: flex-start; padding: 16px; gap: 20px; min-height: auto; text-align: left; border-left: 4px solid var(--danger);" onclick="showProductDetails('${p.ean}')">
                                            ...
                                            <div style="flex: 1;">
                                                <div style="font-weight: 700; color: white; font-size: 0.9rem; margin-bottom: 4px;">${p.descricao_base || "Sem Descri\xE7\xE3o"}</div>
...
const PRODUCT_STOCK_LOCATION_FILTERS = [
  ...
  { key: "DISPONIVEL", label: "Dispon\xEDvel", locals: ["TERREO", "MOSTRUARIO", "PRIMEIRO_ANDAR"] },
  { key: "TERREO", label: "T\xE9rreo", locals: ["TERREO"] },
  { key: "MOSTRUARIO", label: "Mostru\xE1rio", locals: ["MOSTRUARIO"] },
  { key: "PRIMEIRO_ANDAR", label: "1\xBA Andar", locals: ["PRIMEIRO_ANDAR"] },
...
function getStockLocationQty(productId, filterKey = currentProductStockFilter) {
  ...
  return appData.estoque.reduce((total, item) => {
    ...
    const qty = saldoTotal !== void 0 && saldoTotal !== null ? parseFloat(String(saldoTotal).replace(",", ".")) : parseFloat(String(item.saldo_disponivel || 0).replace(",", ".")) + parseFloat(String(item.saldo_reservado || 0).replace(",", ".")) + parseFloat(String(item.saldo_em_transito || 0).replace(",", "."));
...
function getProductSearchStatusText(queryRaw = "") {
  ...
  if (query && filter.key !== "TODOS") return `Busca: ${query} \xB7 Local: ${filter.label}`;
  if (query) return `Busca: ${query} \xB7 Local: Todos`;
...
async function renderSearchScreen(push = true) {
  if (currentScreen === "search" && document.getElementById("search-input")) {
    console.log("[BUSCA MOBILE DEBUG] re-render ignorado (j\xE1 ativo)");
    await ensureProdutosLoaded();
            ...
            <main class="container product-search-center">
                ...
                <div class="search-filters-section">
                    <div class="section-header">
                        <h2 class="section-subtitle">FILTROS R\xC1PIDOS</h2>
                ...
                <div id="search-results" class="product-search-results">
                    <div class="product-search-results-shell">
                        <div id="search-initial-state" class="search-empty-state">
                            ...
                            <div class="empty-state-content">
                                ...
                                <p>Localize produtos instantaneamente por descri\xE7\xE3o, EAN, SKU ou ID.</p>
                            </div>
                        ...
                        ${renderProductSearchSidePanel(appData.products?.length || 0, "")}
                    </div>
                </div>
            </main>
        </div>
...
async function startScanner(isPicking = false, isConference = false, isInventory = false, isGarantia = false, isEdit = false) {
  ...
  try {
    ...
    showToast("C\xE2mera em uso ou n\xE3o dispon\xEDvel. Tente novamente.");
...
async function showScannerFeedback(type, containerId = "scanner-container") {
  ...
  if (type === "success") {
    ...
    feedback.style.background = "rgba(254, 240, 138, 0.3)";
    icon.innerText = "check_circle";
    icon.style.color = "#854d0e";
    const feedbackText = document.createElement("div");
    feedbackText.innerText = "PRODUTO OK";
    feedbackText.style.position = "absolute";
    feedbackText.style.top = "15%";
    feedbackText.style.background = "#fef08a";
    feedbackText.style.color = "#854d0e";
    ...
    container.style.borderColor = "#eab308";
...
function showProductDetailsByCode(code) {
  ...
  const product = appData.products.find(
    (p) => p.ean && p.ean.toString().toLowerCase() === searchCode || p.sku_fornecedor && p.sku_fornecedor.toString().toLowerCase() === searchCode || p.id_interno && p.id_interno.toString().toLowerCase() === searchCode || p.col_B && p.col_B.toString().toLowerCase() === searchCode
  );
  if (product) {
    renderProductDetails(product);
  } else {
    playBeep("error");
    showToast(`PRODUTO N\xC3O CADASTRADO: ${code}`);
...
const doPerformSearch = async () => {
  ...
  const productsSource = Array.isArray(appData.products) ? appData.products : [];
  const textResults = query.length >= 2 ? productsSource.filter((p) => p._searchIndex.includes(query)) : productsSource.slice();
  ...
  if (finalResults.length === 0 && queryRaw.length >= 3) {
    ...
    showScanFeedback("warning", isPotentialCode ? "PRODUTO N\xC3O CADASTRADO" : "PRODUTO N\xC3O EXISTE");
  }
};
...
function handleSearchEnter(event2) {
  if (event2.key === "Enter") {
    const rawValue = event2.target.value.trim();
    if (!rawValue) return;
...
function cleanProductSearchText(value) {
  ...
  return String(value).replaceAll("\xE1", "\xE1").replaceAll("\xE2", "\xE2").replaceAll("\xE3", "\xE3").replaceAll("\xE7", "\xE7").replaceAll("\xE9", "\xE9").replaceAll("\xEA", "\xEA").replaceAll("\xED", "\xED").replaceAll("\xF3", "\xF3").replaceAll("\xF4", "\xF4").replaceAll("\xF5", "\xF5").replaceAll("\xFA", "\xFA").replaceAll("\xBA", "\xBA").replaceAll("\xE1", "\xE1").replaceAll("\xE2", "\xE2").replaceAll("\xE3", "\xE3").replaceAll("\xE7", "\xE7").replaceAll("\xE9", "\xE9").replaceAll("\xEA", "\xEA").replaceAll("\xED", "\xED").replaceAll("\xF3", "\xF3").replaceAll("\xF4", "\xF4").replaceAll("\xF5", "\xF5").replaceAll("\xFA", "\xFA").replaceAll("\xBA", "\xBA");
}
...
function getSearchProductPackInfo(product, estoqueTotal) {
  const qtdEmbalagem = parseFloat(String(product?.quantidade_embalagem ?? "").replace(",", "."));
...
function renderSearchProductPackInfo(product, estoqueTotal) {
  ...
  return `
        <span class="stock-pack-pill">
            <span class="stock-pack-cx">${formatStockNumber(pack.caixas)} CX</span>
            ${pack.sobra > 0 ? `<span class="stock-pack-rest">+ ${formatStockNumber(pack.sobra)} UN</span>` : ""}
        </span>
    `;
}
...
function renderSearchProductSellableLocations(product) {
  ...
  const locations = [
    { key: "TERREO", label: "T\xE9rreo" },
    { key: "PRIMEIRO_ANDAR", label: "1\xBA Andar" }
  ].map((location) => ({
    ...location,
  ...
  return `
        <div class="stock-sellable-locations" aria-label="Estoque dispon\xEDvel por local">
...
function renderProductSearchSidePanel(resultCount, queryRaw = "") {
  ...
  return `
        <aside class="product-search-side-panel">
            <section class="product-side-card product-side-summary">
                <span class="product-side-title">RESUMO DA BUSCA</span>
            ...
            <section class="product-side-card">
                ...
                <div class="product-side-current">Atual: <strong>${activeFilter.label}</strong></div>
            </section>
            ...
            <section class="product-side-card">
                ...
                <div class="product-side-query">
                    <span class="material-symbols-rounded">check_circle</span>
                    <div>
                        <strong>${query ? cleanProductSearchText(query) : "Nome, ID, EAN ou SKU"}</strong>
                        <small>${query ? "termo pesquisado" : "digite ou bipe um produto"}</small>
                    </div>
                </div>
            </section>

            <section class="product-side-card product-side-tip">
                <div class="product-side-heading">
                    <span class="material-symbols-rounded">lightbulb</span>
                    <span>Dica</span>
                </div>
                <p>Selecione o setor para ler o saldo do local e a convers\xE3o em caixas.</p>
            </section>
        </aside>
    `;
}
function renderSearchResults(results) {
    ...
    resultsContainer.innerHTML = `
            <div class="product-search-results-shell">
                <div class="search-no-results">
                    ...
                    <p>Verifique a ortografia ou tente termos mais gen\xE9ricos.</p>
                </div>
                ${renderProductSearchSidePanel(0, query)}
            </div>
        `;
    return;
  }
  const cardsHtml = results.map((p, index) => {
    const idInterno = p.id_interno || p.col_A || "-";
    const desc = cleanProductSearchText(p.descricao_completa || p.col_C || "Produto sem descri\xE7\xE3o");
            ...
            <div class="search-result-card ${!isAtivo ? "inactive" : ""}" 
                ...
                <div class="card-main-info">
                    <div class="card-header-row">
                        <span class="card-id"><small>ID INTERNO</small><strong>${idInterno}</strong></span>
                ...
                <div class="card-operational-info">
                    ...
                    <div class="card-price-block">
                        <span class="price-label">PRE\xC7O VAREJO</span>
  ...
  resultsContainer.innerHTML = `
        <div class="product-search-results-shell">
            ...
            ${renderProductSearchSidePanel(results.length, query)}
        </div>
    `;
...
function highlightMatch(text, query) {
  ...
  try {
    ...
    return parts.map(
      (part) => part.toLowerCase() === query.toLowerCase() ? `<span class="search-highlight">${part}</span>` : part
    ).join("");
  } catch (e) {
    return text;
  }
...
function calcularEstoqueDisponivel(estoques) {
  ...
  return estoques.filter((item) => LOCAIS_DISPONIVEIS.includes(normalizarLocal(item.local))).reduce((total, item) => {
...
function calcularEstoqueNaoVendavel(estoques) {
  if (!estoques || !Array.isArray(estoques)) return 0;
  return estoques.filter((item) => LOCAIS_NAO_VENDAVEIS.includes(normalizarLocal(item.local))).reduce((total, item) => {
    const qtd = parseFloat(String(item.saldo_total || item.saldo || 0).replace(",", "."));
    return total + (isNaN(qtd) ? 0 : qtd);
  }, 0);
}
...
function getStockQtyByLocal(estoques, local) {
  ...
  return estoques.filter((item) => normalizeLocal(item.local) === wanted).reduce((total, item) => total + parseStockQty(item.saldo_total ?? item.saldo), 0);
}
...
function renderInventoryQuantitySummary(product, item, expectedQty) {
  ...
  return `
        <span class="inv-expected-stock inv-count-comparison">
            ...
            <span class="${diffQty === 0 ? "is-ok" : diffQty > 0 ? "is-positive" : "is-negative"}"><strong>Diferen\xE7a:</strong> ${diffQty > 0 ? "+" : ""}${formatStockNumber(diffQty)} UN</span>
...
async function renderProductDetails(p) {
  ...
  const stockAlerts = [
    terreoQty === 0 && primeiroAndarQty > 0 ? {
      type: "warning",
      icon: "sync_alt",
      title: "Dispon\xEDvel no 1\xBA andar. Transferir para venda.",
      detail: `${primeiroAndarQty} ${unitLabel} no 1\xBA andar`
    } : null,
    terreoQty === 0 && primeiroAndarQty === 0 && mostruarioQty > 0 ? {
      type: "showroom",
      icon: "visibility",
      title: "Somente mostru\xE1rio dispon\xEDvel. Verificar estado do produto antes da venda.",
      detail: `${mostruarioQty} ${unitLabel} no mostru\xE1rio`
    } : null,
    defeitoQty > 0 ? {
      type: "subtle",
      icon: "build_circle",
    ...
    fullMlQty > 0 ? {
      ...
      detail: "Dep\xF3sito Mercado Livre"
    } : null
  ].filter(Boolean);
  const stockAlertsHTML = stockAlerts.length ? `
        <div class="product-stock-alerts">
            ${stockAlerts.map((alert2) => `
                <div class="product-stock-alert ${alert2.type}">
                    <span class="material-symbols-rounded">${alert2.icon}</span>
                    <div>
                        <strong>${alert2.title}</strong>
                        ${alert2.detail ? `<small>${alert2.detail}</small>` : ""}
                    </div>
                </div>
            `).join("")}
        </div>
  ...
  const stockLocationsHTML = `
        <div class="product-stock-locations">
            ...
            <div class="product-stock-group">
                <div class="product-stock-group-title">Dispon\xEDvel operacional</div>
                <div class="product-stock-grid">
                    ${renderStockLocationCard("TERREO", "T\xE9rreo", terreoQty, "operational")}
                    ${renderStockLocationCard("PRIMEIRO_ANDAR", "1\xBA Andar", primeiroAndarQty, "operational")}
                </div>
            </div>
            <div class="product-stock-group">
                <div class="product-stock-group-title">Estoque secund\xE1rio / n\xE3o dispon\xEDvel</div>
                <p class="product-stock-group-note">Mostru\xE1rio: venda opcional - verificar estado do produto.</p>
                <div class="product-stock-grid product-stock-grid-secondary">
                    ${renderStockLocationCard("MOSTRUARIO", "Mostru\xE1rio", mostruarioQty, "secondary")}
            ...
            <div class="product-stock-group product-stock-group-external">
                ...
                <p class="product-stock-group-note">Dep\xF3sito Mercado Livre - n\xE3o dispon\xEDvel para venda direta.</p>
                <div class="product-stock-grid">
  ...
  const technicalInfoHTML = attrs.length ? renderInfoBlock("T\xE9cnico", attrs.map((attr) => ({
  ...
  const operationalInfoHTML = renderInfoBlock("Operacional", [
    { label: "Estoque m\xEDnimo", value: p.estoque_minimo },
    { label: "Unidade", value: p.unidade },
    { label: "Qtd. embalagem", value: p.quantidade_embalagem },
    { label: "M\xEDnimo atacado", value: p.quantidade_minima_atacado },
    { label: "Status", value: p.status }
  ]);
  const adminInfoHTML = renderInfoBlock("Administrativo", [
    { label: "ID interno", value: idInterno },
    { label: "Descri\xE7\xE3o completa", value: p.descricao_completa },
    { label: "Manual PDF", value: pdfUrl },
    { label: "Observa\xE7\xF5es", value: p.observacoes }
  ]);
  app.innerHTML = `
                ...
                <div class="product-detail-card">
                    ...
                    <div class="product-detail-header">
                        ...
                        <div class="product-detail-title-block">
                            <h1 class="product-detail-title">${p.descricao_completa || p.descricao_base || "Sem descri\xE7\xE3o"}</h1>
                    ...
                    <!-- CARDS DE PRE\xC7O E ESTOQUE TOTAL -->
                    <div class="product-detail-prices">
                        <div class="product-price-card product-price-card-varejo">
                            ...
                            <div class="product-price-label">PRE\xC7O DE VENDA (VAREJO)</div>
                            <div class="product-price-value product-price-main">${formatPrice(p.preco_varejo)}</div>
                        </div>
                        <div class="product-price-card product-stock-main-card ${stockStatusClass}" title="T\xE9rreo + 1\xBA Andar">
                            ...
                            <div class="product-price-label">ESTOQUE DISPON\xCDVEL</div>
                        ...
                        <div class="product-price-card product-price-card-atacado collapsible-mobile">
                            ...
                            <div class="product-price-label">PRE\xC7O ATACADO</div>
                            <div class="product-price-value product-price-atacado">${formatPrice(p.preco_atacado)}</div>
                        </div>
                    </div>

                    <!-- ALERTA DE TRANSFER\xCANCIA (1\xBA ANDAR) -->
                    ${packagingHTML}
                    ${stockAlertsHTML}
                    ${stockLocationsHTML}
                    ${false ? (() => {
    ...
    if (false) {
      return `
                            <div style="margin-top: 20px; padding: 14px; background: rgba(251, 191, 36, 0.1); border: 1px solid rgba(251, 191, 36, 0.2); border-radius: 14px; display: flex; align-items: center; gap: 12px;">
                                ...
                                <div>
                                    <div style="color: #fbbf24; font-weight: 800; font-size: 0.85rem;">SEM ESTOQUE NO T\xC9RREO/MOSTRU\xC1RIO</div>
                                    <div style="color: rgba(255,255,255,0.7); font-size: 0.75rem;">Dispon\xEDvel no 1\xBA andar: <b>${p1}</b></div>
    ...
    return `
                                        <div class="related-product-card" onclick="renderProductDetails(${JSON.stringify(eq).replace(/"/g, "&quot;")})" style="background: rgba(255,255,255,0.03); padding: 14px; border-radius: 16px; display: flex; align-items: center; gap: 14px; cursor: pointer; border: 1px solid rgba(255,255,255,0.05); transition: all 0.2s ease;">
                                            ...
                                            <div style="flex: 1; min-width: 0;">
                                                ...
                                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                                    <span style="font-size: 0.8rem; color: #4ade80; font-weight: 700;">${formatPrice(eq.preco_varejo)} \xE7 ${eqStock} disp.${eqMostruario > 0 ? ` \xE7 ${eqMostruario} most.` : ""}</span>
                    ...
                    <!-- BLOCO + INFORMA\xC7\xD5ES (RECOLH\xCDVEL) -->
                        ...
                        <div id="more-info-content" class="hidden" style="padding: 20px 10px 0;">
                            ...
                            <div style="margin-bottom: 24px; padding: 16px; background: rgba(0,0,0,0.2); border-radius: 14px; border: 1px dashed rgba(255,255,255,0.1);">
                                <div id="custo-locked" style="display: flex; align-items: center; justify-content: space-between;">
                                    <div>
                                        <span style="font-size: 0.75rem; color: var(--muted); display: block; margin-bottom: 4px;">PRE\xC7O DE CUSTO</span>
                            ...
                            <div class="product-detail-id-section" style="background: transparent; padding: 0; border: none; margin-bottom: 24px;">
                                ...
                                <div class="product-id-item">
                                    <span class="product-id-label">M\xEDnimo Atacado</span>
                                    <span class="product-id-value">${p.quantidade_minima_atacado} ${p.unidade || "UN"}</span>
                                </div>
                                ` : ""}
                                ${p.estoque_minimo ? `
                                <div class="product-id-item">
                                    <span class="product-id-label">Estoque M\xEDnimo</span>
                                    <span class="product-id-value">${p.estoque_minimo}</span>
                                </div>
                                ` : ""}
                            </div>

                            <!-- ATRIBUTOS T\xC9CNICOS -->
                            ${attrs.length > 0 ? `
                            <div class="product-attrs-section" style="margin-bottom: 24px;">
                                <div class="product-attrs-title" style="font-size: 0.8rem; color: var(--muted); text-transform: uppercase; margin-bottom: 12px; font-weight: 800;">Atributos T\xE9cnicos</div>
                            ...
                            <div class="product-obs-section" style="margin-bottom: 24px;">
                                <div class="product-obs-title" style="font-size: 0.8rem; color: var(--muted); text-transform: uppercase; margin-bottom: 8px; font-weight: 800;">Observa\xE7\xF5es Internas</div>
  ...
  app.innerHTML = `
        <div class="dashboard-screen fade-in internal">
            ...
            <main class="container product-form-screen">
                ...
                <div class="form-grid">
                    <div class="form-section-title">Identifica\xE7\xE3o</div>
                    <div class="input-group">
                        <label>ID Interno (Autom\xE1tico)</label>
                    ...
                    <div class="input-group">
                        <label>EAN / C\xF3digo de Barras</label>
                        <input type="text" id="add-ean" class="input-field" placeholder="EAN13" value="${initialEan}">
                    </div>
                    <div class="input-group">
                        <label>SKU Fornecedor</label>
                        <input type="text" id="add-sku" class="input-field" placeholder="C\xF3digo do Fornecedor">
                    </div>
                    <div class="input-group full-width">
                        <label>Descri\xE7\xE3o Base</label>
                        <input type="text" id="add-desc" class="input-field" placeholder="Nome principal do produto">
                    </div>

                    <div class="form-section-title">Caracter\xEDsticas</div>
                    <div class="input-group">
                        <label>Marca</label>
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" id="add-cat" class="input-field" placeholder="Ex: Suspens\xE3o">
                    </div>
                    <div class="input-group">
                    ...
                    <div class="input-group">
                        ...
                        <select id="add-uni" class="input-field">
                            ...
                            <option value="PC">PC - Pe\xE7a</option>
                    ...
                    <div class="form-section-title">Atributos T\xE9cnicos (JSON)</div>
                    <div class="input-group full-width">
                    ...
                    <div class="form-section-title">M\xEDdia e Documenta\xE7\xE3o</div>
                    <div class="input-group full-width">
...
async function saveNewProduct() {
  ...
  if (attrsInput) {
    try {
      ...
      showToast("JSON de atributos inv\xE1lido");
      return;
    }
  }
  ...
  if (!product.descricao_base) {
    showToast("A descri\xE7\xE3o base \xE9 obrigat\xF3ria.");
    return;
  }
  ...
  try {
    ...
    const { data, error } = await client.from("produtos").insert([product]).select("*").single();
    if (error) throw error;
...
function markRemoveImage() {
  ...
  if (preview) {
    preview.innerHTML = '<span style="color: #E30613; font-size: 0.8rem;">Imagem ser\xE1 removida ao salvar</span>';
  }
  console.log("[REMOVE] Imagem marcada para remo\xE7\xE3o");
}
function markRemovePDF() {
  removePDFFlag = true;
  const preview = document.getElementById("edit-pdf-preview");
  if (preview) {
    preview.innerHTML = '<span style="color: #E30613; font-size: 0.8rem;">PDF ser\xE1 removido ao salvar</span>';
  }
  console.log("[REMOVE] PDF marcado para remo\xE7\xE3o");
}
async function saveEditProduct(originalId) {
  ...
  try {
    const client = window.supabaseClient;
    if (!client) throw new Error("Supabase client nao encontrado");
    const { data, error } = await client.from("produtos").update(product).eq("id_interno", originalId).select("*").single();
    if (error) throw error;
...
function renderEditProductFormByEan(code) {
  ...
  const p = (appData.products || []).find(
    (prod) => String(prod.ean || "").trim() === lookup || String(prod.id_interno || prod.col_A || "").trim() === lookup || String(prod.sku_fornecedor || prod.sku || "").trim() === lookup
  ) || (window.currentProductDetailForEdit && (String(window.currentProductDetailForEdit.ean || "").trim() === lookup || String(window.currentProductDetailForEdit.id_interno || window.currentProductDetailForEdit.col_A || "").trim() === lookup || String(window.currentProductDetailForEdit.sku_fornecedor || window.currentProductDetailForEdit.sku || "").trim() === lookup) ? window.currentProductDetailForEdit : null);
  if (p) {
    renderEditProductForm(p);
  } else {
    console.warn("[EDIT] Produto n\xE3o encontrado para edi\xE7\xE3o:", code);
    showToast("N\xE3o foi poss\xEDvel abrir a edi\xE7\xE3o deste produto.", "warning");
  }
}
        ...
        <div class="dashboard-screen fade-in internal">
            ...
            <main class="container" style="display: flex; flex-direction: column; align-items: center; padding-top: 60px;">
                ...
                <div style="margin-top: 40px; text-align: center; color: var(--muted);">
                    ...
                    <p style="font-size: 1.1rem; font-weight: 600;">M\xF3dulo em desenvolvimento</p>
                    <p style="font-size: 0.85rem; margin-top: 8px;">Aguarde as pr\xF3ximas atualiza\xE7\xF5es.</p>
                </div>
            </main>
        </div>
    `;
                ...
                <div class="form-grid">
                    ...
                    <div class="input-group">
                        ...
                        <select id="edit-uni" class="input-field">
                            ...
                            <option value="PC" ${p.unidade === "PC" ? "selected" : ""}>PC - Pe\xE7a</option>
...
function handleBackgroundImageUpload(event2, deviceType) {
  const file = event2.target.files[0];
  if (!file) return;
  if (!file.type.startsWith("image/")) {
    showToast("Por favor, selecione uma imagem v\xE1lida.");
    return;
  }
...
function handleFontUpload(event2) {
  const file = event2.target.files[0];
  if (!file) return;
  const validExtensions = [".ttf", ".otf", ".woff", ".woff2"];
  const ext = "." + file.name.split(".").pop().toLowerCase();
  if (!validExtensions.includes(ext)) {
    showToast("Por favor, selecione um arquivo de fonte v\xE1lido (TTF, OTF, WOFF, WOFF2).");
    return;
  }
  const reader = new FileReader();
  reader.onload = function(e) {
...
function handleLoginBgImage(event2) {
  const file = event2.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = function(e) {
...
function resetLoginVisual() {
  ...
  request.onsuccess = function() {
    showToast("Visual resetado para padr\xE3o!");
    renderConfigSubMenu();
  };
  request.onerror = function() {
    showToast("Visual resetado para padr\xE3o!");
  ...
  app.innerHTML = `
        <div class="dashboard-screen internal fade-in financeiro-screen module-screen" style="background: var(--bg); min-height: 100vh;">
            ...
            <main class="container" style="padding-top:80px;text-align:center;">
                <p style="color:var(--muted);margin-top:40px;">M\xF3dulo em desenvolvimento.</p>
            </main>
        </div>
    `;
}
function getChannelConfig(label) {
  ...
  if (l.includes("PDV") || l.includes("BALC\xC3O")) return { icon: "store", color: "pdv", svgIcon: channel3DIcons.pdv };
  return { icon: "storefront", color: "pdv", svgIcon: channel3DIcons.pdv };
}
async function renderPickMenu() {
  if (isModoRapidoAtivo()) {
    showToast("Modo r\xE1pido ativo. Use Confer\xEAncia/Sa\xEDda direta.", "info");
  }
  const currentUser = localStorage.getItem("currentUser");
  document.body.classList.remove("menu-active");
...
function renderPickHistory() {
  const currentUser = localStorage.getItem("currentUser");
  const history2 = (appData.separacao || []).sort((a, b) => new Date(b.criado_em) - new Date(a.criado_em));
  app.innerHTML = `
                <div class="dashboard-screen fade-in internal picking-screen">
                    ...
                    <main class="container">
                        <div class="sub-menu-header">
                            <h2 style="font-size: 1.2rem; font-weight: 700;">HIST\xD3RICO DE SEPARA\xC7\xC3O</h2>
                        </div>

                        ${history2.length === 0 ? `
                            <div style="text-align: center; padding: 60px 20px; background: var(--surface); border-radius: 24px; border: 1px dashed rgba(255,255,255,0.1);">
                                <span class="material-symbols-rounded" style="font-size: 48px; color: var(--muted); margin-bottom: 16px;">history</span>
                                <p style="color: var(--muted);">Nenhuma separa\xE7\xE3o encontrada.</p>
                            </div>
                        ` : `
                            <div style="display: flex; flex-direction: column; gap: 12px; padding-bottom: 40px;">
                                ${history2.map((item) => `
                                    <div style="background: var(--surface); padding: 16px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05);">
                                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
                                            ...
                                            <div style="background: ${item.status === "CONCLU\xCDDO" ? "rgba(34, 197, 94, 0.1)" : "rgba(234, 179, 8, 0.1)"}; color: ${item.status === "CONCLU\xCDDO" ? "#22c55e" : "#eab308"}; padding: 2px 8px; border-radius: 4px; font-size: 0.65rem; font-weight: 800;">
...
Para iniciar em ${channelLabel}, voc\xEA deve descartar o rascunho anterior.

Deseja LIMPAR o rascunho de ${draft.channelLabel} e come\xE7ar ${channelLabel}?`;
        if (confirm(msg)) {
          localStorage.removeItem("draft_pick_session");
          console.log(`[PICKING DEBUG] rascunho de ${draft.channelLabel} descartado`);
        } else {
          if (confirm(`Deseja RETOMAR a sess\xE3o de ${draft.channelLabel} agora?`)) {
...
function renderPickingScreen(sessionId, channelId, channelLabel, channelColor) {
  ...
  currentPickingContext = {
    ...
    createdAt: currentPickingContext?.createdAt || (/* @__PURE__ */ new Date()).toISOString()
  };
  app.innerHTML = `
                ...
                <div class="product-search-container">
                    ...
                    <div class="screen-mini-title" style="margin-top: 36px; margin-bottom: 28px;">
                        ...
                        <div style="display: flex; flex-direction: column;">
                            <span>${(channelLabel || "SEPARA\xC7\xC3O").toUpperCase()}</span>
                        </div>
                    </div>

                    <!-- CAMPO DE BUSCA PADR\xC3O -->
                    <div class="product-search-bar">
                        <span class="material-symbols-rounded">search</span>
...
function findProductInLocalCacheByCode(cleanCode) {
  ...
  const product = (appData.products || []).find(
    (p) => normalizePickCode(p.ean) === code || normalizePickCode(p.id_interno || p.col_a || p.col_A) === code || normalizePickCode(p.sku_fornecedor || p.sku) === code
  );
  if (product) {
...
function upsertProductIntoLocalCache(product) {
  ...
  const idx = appData.products.findIndex(
    (p) => normalizePickCode(p.ean) === normalizePickCode(indexed.ean) || normalizePickCode(p.id_interno || p.col_a || p.col_A) === normalizePickCode(indexed.id_interno || indexed.col_a || indexed.col_A)
  );
  ...
  if (product) {
    ...
    if (stock < currentDraftQty + 1) {
      if (!allowNegative) {
        ...
        if (!window.pickNegativeStockWarnings) window.pickNegativeStockWarnings = /* @__PURE__ */ new Set();
    ...
    if (existingItem) {
      ...
      existingItem.scanTime = (/* @__PURE__ */ new Date()).toLocaleTimeString();
    } else {
      currentSessionItems.unshift({
        ...product,
        qty: 1,
        scanTime: (/* @__PURE__ */ new Date()).toLocaleTimeString()
      });
    }
    ...
    draft.createdAt = draft.createdAt || (/* @__PURE__ */ new Date()).toISOString();
    ...
    playBeep("error");
    showToast(`PRODUTO N\xC3O CADASTRADO: ${ean}`);
  }
  input.value = "";
  updatePickItemsList();
...
async function finishPickingSession(sessionId, channelId, channelLabel, channelColor) {
  ...
  try {
    ...
    const pickingData = {
      ...
      canal_id: channelId,
      canal_nome: channelLabel,
      data_separacao: (/* @__PURE__ */ new Date()).toLocaleDateString("pt-BR"),
      status: "em_separacao",
      criado_por: currentUser,
      criado_em: now,
      finalizado_em: now,
    ...
    showToast("Erro ao processar separa\xE7\xE3o!");
  } finally {
    isFinalizing = false;
    const submitBtn2 = document.querySelector(`button[onclick^="finishPickingSession"]`);
    if (submitBtn2) {
      submitBtn2.disabled = false;
      submitBtn2.innerHTML = '<span class="material-symbols-rounded">check_circle</span> Finalizar Separa\xE7\xE3o';
    }
  }
}
...
async function savePickResultFinal(sessionId, channelId, channelLabel, channelColor) {
  if (isFinalizing) return;
  isFinalizing = true;
  showToast("Finalizando separa\xE7\xE3o...");
  try {
    ...
    localStorage.removeItem("draft_pick_session");
    showToast(finalResult?.queued ? `Separacao ${sessionId} salva localmente para sincronizar.` : `Separacao ${sessionId} enviada para conferencia!`);
    renderMenu();
  } catch (e) {
...
async function renderPackMenu() {
  ...
  const modoRapidoAtivo = isModoRapidoAtivo();
  if (modoRapidoAtivo) {
    showToast("Acesso negado: Confer\xEAncia desativada no Modo R\xE1pido.");
    renderMenu();
    return;
  }
  try {
...
function renderPackHistory() {
  const currentUser = localStorage.getItem("currentUser");
  const history2 = (appData.conferencia || []).sort((a, b) => new Date(b.conferido_em) - new Date(a.conferido_em));
  app.innerHTML = `
                <div class="dashboard-screen fade-in internal pack-screen">
                    ...
                    <main class="container">
                        <div class="sub-menu-header">
                            <h2 style="font-size: 1.2rem; font-weight: 700;">HIST\xD3RICO de CONFER\xCANCIA</h2>
                        </section>

                        ${history2.length === 0 ? `
                            <div style="text-align: center; padding: 60px 20px; background: var(--surface); border-radius: 24px; border: 1px dashed rgba(255,255,255,0.1);">
                                <span class="material-symbols-rounded" style="font-size: 48px; color: var(--muted); margin-bottom: 16px;">history</span>
                                <p style="color: var(--muted);">Nenhuma confer\xEAncia encontrada.</p>
                            </div>
                        ` : `
                            <div style="display: flex; flex-direction: column; gap: 12px; padding-bottom: 40px;">
                                ${history2.map((item) => `
...
function renderPackSessionsList(channelName) {
  ...
  const getSessionItemCount = (session) => {
    ...
    const items = (appData.separacao_itens || []).filter(
      (item) => String(item.separacao_id || item.rom_id || item.col_a || "") === String(sid)
    );
    return items.length || "-";
  };
                    ...
                    <main class="container pack-sessions-workspace">
                        ...
                        ${activeSessions.length === 0 ? `
                            <div class="pack-sessions-empty">
                                ...
                                <strong>Nenhuma sess\xE3o aberta</strong>
                                <p>Quando uma separa\xE7\xE3o for finalizada para confer\xEAncia, ela aparecer\xE1 aqui.</p>
                            </div>
                        ` : `
                                ...
                                <article class="pack-session-card" onclick="renderPackSessionDetails('${sid}')">
                                    <div class="pack-session-card-main">
                                        ...
                                        <div class="pack-session-card-text">
                                            <span class="pack-session-card-kicker">Separa\xE7\xE3o</span>
                                            <strong>${sid || "-"}</strong>
...
function deletePickingSession(sessionId, channelName) {
  if (!confirm(`Tem certeza que deseja excluir a separa\xE7\xE3o ${sessionId}?
Esta a\xE7\xE3o n\xE3o pode ser desfeita.`)) {
  ...
  setActivePickSessions(activeSessions);
  showToast(`Separa\xE7\xE3o ${sessionId} exclu\xEDda.`);
  if (channelName) {
...
async function renderPackSessionDetails(sessionId) {
  ...
  const separacaoSession = (appData.separacao || []).find(
    (s) => (s.separacao_id || s.col_a) === sessionId
  );
  if (separacaoSession) {
    ...
    if (criadoPor && criadoPor === usuarioAtual) {
      showToast("Esta confer\xEAncia deve ser realizada por outro usu\xE1rio. O mesmo usu\xE1rio que fez a separa\xE7\xE3o n\xE3o pode conferir esta sess\xE3o.", "error");
      playBeep("error");
      return;
    }
  }
  ...
  try {
    ...
    const conferenceRows = Object.keys(groupedExpected).length > 0 ? Object.values(groupedExpected) : session && Array.isArray(session.conferenceRows) ? session.conferenceRows : [];
    currentPackSession = {
      id: sessionId,
      items: expectedItems,
      pickingData: {
...
function renderPackSessionFrame(sessionId, currentUser, channelColorClass = "", channelName = "") {
  ...
  const title = channelName ? channelName.toUpperCase() : "CONFER\xCANCIA";
  app.innerHTML = `
                ...
                <div class="pack-main-panel">
                    <div class="pack-session-header" id="pack-session-header">
                        <div class="pack-session-title-block">
                            ...
                            <div>
                                <div class="pack-session-kicker">Confer\xEAncia / Pack</div>
                                <h2>${title}</h2>
                                <span>Separa\xE7\xE3o: ${sessionId}</span>
                            </div>
                        </div>
...
function getProductForConferenceRow(row) {
  return (appData.products || []).find(
    (p) => row.ean && String(p.ean || "") === String(row.ean) || row.id_interno && String(p.id_interno || p.col_A || p.col_a || "") === String(row.id_interno)
  ) || {};
}
...
function updatePackChrome() {
  ...
  const statusLabel = stats.checkedQty > 0 ? "Em confer\xEAncia" : "Aberta";
...
function renderPackItemsListHTML() {
  ...
  if (rowsToShow.length === 0) {
    ...
    return `<div class="pack-empty-state">Clique no campo acima e bipe um produto para iniciar a confer\xEAncia cega.</div>`;
  }
    ...
    return `
            <div class="fade-in conference-item pack-item-card success">
                ...
                <div class="pack-item-product">
                    <span>${row.descricao || product.descricao_completa || product.descricao_base || "Produto sem descri\xE7\xE3o"}</span>
...
function openManualAddProduct(callback) {
  ...
  if (!term) return;
  const product = appData.products.find(
    (p) => p.ean && p.ean.toString() === term || p.id_interno && p.id_interno.toString() === term || p.sku_fornecedor && p.sku_fornecedor.toString() === term
  );
  if (!product) {
    playBeep("error");
...
function addPackScan(scannedEan = null) {
  ...
  if (!ean) return;
  let row = currentPackSession.conferenceRows.find(
    (r) => r.ean && r.ean.toString() === ean || r.id_interno && r.id_interno.toString() === ean
  );
  if (row) {
    playBeep("success");
    console.log(`[BIP CONTINUO DEBUG] produto adicionado (confer\xEAncia): ${row.id_interno || ean}`);
    row.qtd_conferida++;
    if (row.qtd_conferida === row.qtd_separada) {
      row.divergencia = "OK";
    } else if (row.qtd_conferida > row.qtd_separada) {
    ...
    playBeep("error");
    showToast("Item n\xE3o encontrado nesta separa\xE7\xE3o! Registrado como SOBRA.");
    const product = appData.products.find(
      (p) => p.ean && p.ean.toString() === ean || p.sku_fornecedor && p.sku_fornecedor.toString() === ean || p.id_interno && p.id_interno.toString() === ean || p.col_a && p.col_a.toString() === ean || p.col_A && p.col_A.toString() === ean
    );
    row = {
      rom_id: currentPackSession.id,
      id_interno: product ? product.id_interno || product.col_a || product.col_A || "" : "",
      ean,
      descricao: product ? product.descricao_base || product.col_aa || "PRODUTO N\xC3O IDENTIFICADO" : "PRODUTO N\xC3O IDENTIFICADO",
      qtd_separada: 0,
      qtd_conferida: 1,
      divergencia: "SOBRA",
      conferido_por: "",
...
function renderConferenceCorrection() {
  ...
  let conferenceStatus = "EM CONFER\xCANCIA";
  ...
  if (hasDivergence) {
    setTimeout(() => {
      ...
      if (firstError) {
        ...
        setTimeout(() => firstError.style.boxShadow = "", 2e3);
      }
    }, 300);
  }
  app.innerHTML = `
                <div class="dashboard-screen fade-in internal">
                    ...
                    <main class="container">
                        <div class="sub-menu-header" style="flex-direction: column; align-items: flex-start; gap: 4px;">
                            <div style="font-size: 0.7rem; color: var(--primary); font-weight: 800; letter-spacing: 0.1em;">RESULTADO DA CONFER\xCANCIA</div>
                            <div style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <div style="margin-bottom: 24px; padding: 20px; border-radius: 20px; background: ${hasDivergence ? "rgba(239, 68, 68, 0.05)" : "rgba(34, 197, 94, 0.1)"}; border: 1px solid ${hasDivergence ? "rgba(239, 68, 68, 0.2)" : "rgba(34, 197, 94, 0.2)"}; text-align: center;">
                            ...
                            <h3 style="font-size: 1.1rem; font-weight: 700; color: white;">
                                ${hasDivergence ? "Aten\xE7\xE3o: Diverg\xEAncia Detectada" : "Fluxo Validado com Sucesso"}
                            </h3>
                            <p style="font-size: 0.8rem; color: var(--muted); margin-top: 4px;">
                                ${hasDivergence ? "H\xE1 uma diferen\xE7a entre o separado e o conferido. Ajuste obrigat\xF3rio para liberar." : "Tudo em ordem. O status final ser\xE1 gravado como CONFERIDO."}
                            </p>
                        </div>

                        <div style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 30px;">
                            ${currentPackSession.conferenceRows.map((row, index) => {
    if (row.divergencia === "OK" && !hasDivergence) return "";
    ...
    return `
                        ...
                        <div style="margin: 20px 0; padding: 16px; background: rgba(234, 179, 8, 0.1); border-radius: 12px; border: 1px solid rgba(234, 179, 8, 0.3); text-align: center;">
                            <p style="color: #facc15; font-weight: 600; margin-bottom: 16px;">Deseja corrigir as diverg\xEAncias?</p>
                            <button onclick="confirm('Clique OK para confirmar a corre\xE7\xE3o e prosseguir') && renderConferenceCorrection()" class="btn-action" style="width: 100%; background: #f59e0b;">
                                <span class="material-symbols-rounded">edit</span>
                        ...
                        <!-- Bot\xE3o Adicionar Produto Extra -->
                        ...
                        <div style="display: flex; flex-direction: column; gap: 12px; padding-bottom: 40px;">
                            ...
                            <button class="btn-action" id="btn-finish-atomic"
                                    ...
                                    ${hasDivergence ? `disabled onclick="showToast('Corrija as diverg\xEAncias para finalizar', 'warning')"` : 'onclick="confirmFinishConference()"'}>
                                <span class="material-symbols-rounded">check_circle</span>
                            ...
                            <div style="text-align: center; color: #ef4444; font-size: 0.75rem; font-weight: 800; padding: 18px; background: rgba(239, 68, 68, 0.1); border-radius: 12px; border: 2px solid rgba(239, 68, 68, 0.3);">
                                ...
                                <span style="display: block; font-size: 1rem; letter-spacing: 0.1em;">CORRE\xC7\xC3O PENDENTE</span>
...
function adjustConferenceRow(index, delta) {
  ...
  if (prevDivergence !== "OK" && row.divergencia === "OK") {
    ...
    console.log("Diverg\xEAncia corrigida logada:", logEntry);
  }
  let activeSessions = getActivePickSessions();
...
async function finishConferenceSession() {
  const hasDivergence = currentPackSession.conferenceRows.some(
    (row) => parseFloat(row.qtd_conferida || 0) !== parseFloat(row.qtd_separada || 0)
  );
  if (hasDivergence) {
    showToast("Diverg\xEAncia detectada! Abrindo tela de corre\xE7\xE3o.", "warning");
    playBeep("error");
    renderConferenceCorrection();
  } else {
    if (confirm("Confer\xEAncia perfeita! Deseja finalizar e dar baixa no estoque agora?")) {
...
function startFastPackSession(channelLabel, channelColor) {
  ...
  currentPackSession = {
    ...
    pickingData: {
      ...
      observacao: "MODO CONFER\xCANCIA DIRETA"
    },
    conferenceRows: [],
    isFastMode: true
  };
...
function isRetryableConferenceSyncError(error) {
  ...
  return error?.name === "TypeError" || error?.name === "AbortError" || retryableMessages.some((text) => message.includes(text));
}
...
function startFastMode() {
  console.log("[FastMode] Iniciando modo r\xE1pido...");
  const config = getAppConfig();
  setAppConfig({ ...config, modo_rapido: true });
  ensureFreshData(() => renderPickMenu());
}
function stopFastMode() {
  console.log("[FastMode] Desativando modo r\xE1pido...");
  const config = getAppConfig();
...
async function ensureKitLampadaLoaded(force = false) {
  if (!force && Array.isArray(window.kitLampadaCache) && window.kitLampadaCache.length > 0) {
    console.log("[KIT] cache j\xE1 existe:", window.kitLampadaCache.length);
  ...
  const client = window.supabaseClient;
  if (!client) {
    console.error("[KIT] Supabase client n\xE3o inicializado");
    return [];
  }
  try {
    const { data, error } = await client.from("kit_lampada").select("*").order("kit_lampada_id", { ascending: true });
            ...
            <main class="container product-search-center">
                <div id="kit-content-area">
                    <div style="text-align: center; padding: 100px 40px; color: var(--muted);">
                        ...
                        <p style="font-size: 0.9rem; opacity: 0.7;">Sincronizando guia de l\xE2mpadas com o Supabase</p>
                    </div>
                </div>
            </main>
        </div>
  ...
  try {
    ...
    if (!data || data.length === 0) {
      contentArea.innerHTML = `
                <div style="text-align: center; padding: 80px 40px;">
                    ...
                    <h2 style="color: white; margin-bottom: 12px;">Sem dados dispon\xEDveis</h2>
    ...
    contentArea.innerHTML = `
            <div style="text-align: center; padding: 80px 40px;">
                ...
                <h2 style="color: white; margin-bottom: 12px;">Erro ao carregar Kit L\xE2mpada</h2>
                <p style="color: var(--muted); margin-bottom: 24px; max-width: 400px; margin-left: auto; margin-right: auto;">
                    N\xE3o foi poss\xEDvel conectar ao banco de dados. Verifique permiss\xF5es/RLS do Supabase.
                </p>
                <button class="primary-btn" onclick="renderGuiaLampada(false)" style="margin: 0 auto; display: flex; align-items: center; gap: 8px;">
                    <span class="material-symbols-rounded">refresh</span>
                    Tentar Novamente
...
function handleKitLampadaSearch(term) {
  ...
  if (filtered.length > 0) {
    ...
    showScanFeedback("warning", "ITEM N\xC3O ENCONTRADO");
  }
  ...
  if (results.length === 0) {
    resultsContainer.innerHTML = `
            <div style="text-align: center; padding: 40px; color: var(--muted);">
                ...
                <p>Nenhum ve\xEDculo encontrado para "${term}"</p>
            </div>
        `;
    return;
  }
    ...
    return `
                    <div class="kit-lamp-card" onclick='renderKitDetailsCard(${itemData})'>
                        <div class="product-img-container" style="width: 70px; height: 70px; border-radius: 12px; background: rgba(0,0,0,0.05); display: flex; align-items: center; justify-content: center; flex-shrink: 0; border: 1px solid rgba(0,0,0,0.05);">
                            ${item.url ? `<img src="${item.url}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 10px;" onerror="this.outerHTML='<span class=&quot;material-symbols-rounded&quot; style=&quot;color: #555&quot;>directions_car</span>'">` : `<span class="material-symbols-rounded" style="color: #555">directions_car</span>`}
                        </div>
                        <div style="flex: 1; min-width: 0;">
  ...
  modal.innerHTML = `
        <div class="kit-detail-modal" id="kit-modal-content">
            ...
            <div class="kit-modal-image-area" style="width: 100%; height: 200px; background: #000; display: flex; align-items: center; justify-content: center; cursor: zoom-in;" onclick="openKitImageViewer('${item.url || ""}')">
                ${item.url ? `<img src="${item.url}" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.outerHTML='<span class=&quot;material-symbols-rounded&quot; style=&quot;font-size: 64px; color: var(--muted)&quot;>directions_car</span>'">` : `<span class="material-symbols-rounded" style="font-size: 64px; color: var(--muted)">directions_car</span>`}
            </div>
...
function renderProductSubMenu() {
  ...
  if (!container) {
    console.error("Container principal n\xE3o encontrado para renderProductSubMenu");
    return;
  }
  container.innerHTML = `
                ...
                <div style="padding: 0 20px;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 16px;">
                        ...
                        <div style="background: rgba(255,255,255,0.05); padding: 20px; border-radius: 20px; display: flex; justify-content: space-between; align-items: center; border: 1px solid rgba(255,255,255,0.1);">
                            <div style="flex: 1;">
                                <div style="color: white; font-weight: 700; font-size: 1rem;">SA\xCDDA COM ESTOQUE ZERO</div>
                                <div style="color: var(--muted); font-size: 0.8rem; margin-top: 4px;">Permitir finalizar sa\xEDdas mesmo sem saldo em estoque.</div>
                            </div>
                        ...
                        <div style="background: rgba(255,255,255,0.05); padding: 20px; border-radius: 20px; display: flex; justify-content: space-between; align-items: center; border: 1px solid rgba(255,255,255,0.1);">
                            <div style="flex: 1;">
                                <div style="color: white; font-weight: 700; font-size: 1rem;">MODO R\xC1PIDO</div>
                                <div style="color: var(--muted); font-size: 0.8rem; margin-top: 4px;">Simplifica fluxos e desabilita Separa\xE7\xE3o manual.</div>
                            </div>
                            <div class="toggle-switch">
...
function toggleConfig(key, value) {
  ...
  config[key] = value;
  setAppConfig(config);
  showToast(`Configura\xE7\xE3o atualizada`, "success");
  if (key === "modo_rapido") {
    renderMenu(false);
  }
}
async function renderGarantiaEnvioForm() {
        ...
        <div class="dashboard-screen internal fade-in garantia-screen">
            ...
            <main class="container" style="padding-top: 60px;">
                <div style="padding: 0 20px 40px 20px;">
                    <h2 style="color: white; font-family: 'Fjalla One', sans-serif; margin-bottom: 20px;">ENVIAR PARA GARANTIA / DEVOLU\xC7\xC3O</h2>
                    ...
                    <div class="form-grid" style="background: white; padding: 24px; border-radius: 24px; gap: 16px;">
                        ...
                        <div class="input-group">
                            <label style="color: #666;">TIPO DA OPERA\xC7\xC3O</label>
                            <select id="garantia-tipo-operacao" class="input-field" style="background: #f5f5f5; appearance: none;">
                                <option value="GARANTIA_TECNICA">Garantia T\xE9cnica</option>
                                <option value="TROCA_COMERCIAL">Troca Comercial</option>
                                <option value="DEVOLUCAO_FORNECEDOR">Devolu\xE7\xE3o Fornecedor</option>
                        ...
                        <div class="input-group">
                            <label style="color: #666;">CUSTO UNIT\xC1RIO</label>
                        ...
                        <div class="input-group full-width">
                            ...
                            <select id="garantia-motivo" class="input-field" style="background: #f5f5f5; appearance: none;">
                                <option>Defeito de f\xE1brica</option>
                        ...
                        <div class="input-group full-width">
                            <label style="color: #666;">OBSERVA\xC7\xC3O T\xC9CNICA</label>
                            <textarea id="garantia-observacao" class="input-field" style="background: #f5f5f5; min-height: 80px;" placeholder="Detalhes t\xE9cnicos..."></textarea>
                        </div>
  ...
  searchInput.addEventListener("input", async (e) => {
    ...
    const classification = classifyProductInput(rawValue);
    console.log(`[GARANTIA BUSCA DEBUG] classifica\xE7\xE3o:`, classification);
    let matches = [];
      ...
      dropdown.innerHTML = matches.map((p) => `
                <div class="search-item" data-id="${p.id_interno}" style="padding: 12px 16px; border-bottom: 1px solid #f0f0f0; cursor: pointer; display: flex; align-items: center; gap: 12px; transition: background 0.2s;">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        ...
                        <div style="font-size: 0.7rem; color: #999;">${p.id_interno} | ${p.marca || "S/M"} | Stock: ${appData.estoque?.filter((e2) => e2.id_interno === p.id_interno).reduce((acc, curr) => acc + (curr.saldo_disponivel || 0), 0)}</div>
                    </div>
                </div>
            `).join("");
  ...
  btnSave.addEventListener("click", async () => {
    ...
    const qty = parseFloat(qtyInput.value);
    if (qty <= 0) {
      showToast("Quantidade inv\xE1lida", "warning");
      return;
    }
    const productEstoque = appData.estoque.filter((e) => e.id_interno === selectedProduct.id_interno);
    const sourceStock = productEstoque.find((e) => e.local === selectedSource)?.saldo_disponivel || 0;
    if (qty > sourceStock) {
      showToast(`Estoque insuficiente em ${selectedSource} (Dispon\xEDvel: ${sourceStock})`, "warning");
      return;
    }
        ...
        <div class="dashboard-screen internal fade-in nf-submenu-screen entrada-nf-screen module-screen">
            ...
            <main class="container">
                <div class="menu-grid">
                    ...
                    <div class="menu-card mobile-nav-card" onclick="renderNFPlaceholder('HIST\xD3RICO DE ENTRADAS')">
                        <span class="menu-icon-3d">${menu3DIcons.historico}</span>
                        <span class="label">HIST\xD3RICO DE ENTRADAS</span>
                    </div>
                </div>
            </main>
        </div>
                ...
                <div class="form-grid" style="padding: 0 20px 60px 20px; gap: 24px;">
                    ...
                    <div class="form-section-block">
                        ...
                        <div class="form-grid-inner" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;">
                            <div class="input-group">
                                <label>N\xBA NF</label>
                                <input type="text" id="nf_numero" class="input-field" placeholder="000000">
                            </div>
                            <div class="input-group">
                                <label>S\xE9rie</label>
                                <input type="text" id="nf_serie" class="input-field" placeholder="1">
                            </div>
                            <div class="input-group">
                                <label>Emiss\xE3o</label>
                                <input type="date" id="nf_data_emissao" class="input-field">
                            </div>
                            <div class="input-group">
                                <label>Recebimento</label>
                                <input type="date" id="nf_data_recebimento" class="input-field" value="${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}">
                            </div>
                            <div class="input-group full-width" style="grid-column: span 2;">
                                <label>Fornecedor</label>
                                <input type="text" id="nf_fornecedor" class="input-field" placeholder="Nome Fantasia / Raz\xE3o Social">
                            </div>
                        ...
                        <div class="form-grid-inner" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;">
                            <div class="input-group full-width" style="grid-column: span 2;">
                                ...
                                <select id="compra_pagamento_forma" class="input-field">
                                    <option value="boleto">Boleto Banc\xE1rio</option>
                                    <option value="pix">PIX</option>
                                    <option value="dinheiro">Dinheiro (Esp\xE9cie)</option>
                                    <option value="cartao">Cart\xE3o de Cr\xE9dito/D\xE9bito</option>
                                    <option value="transferencia">Transfer\xEAncia Banc\xE1ria</option>
                                    <option value="outro">Outro</option>
                                </select>
                            </div>
                            <div class="input-group">
                                <label>Condi\xE7\xE3o</label>
                                <select id="compra_pagamento_condicao" class="input-field" onchange="toggleParcelasNf(this.value)">
                                    <option value="a_vista">\xC0 Vista</option>
                            ...
                            <div class="input-group full-width" style="grid-column: span 2;">
                                <label>1\xBA Vencimento</label>
                                <input type="date" id="compra_pagamento_vencimento" class="input-field" value="${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}">
                            </div>
                            <div class="input-group full-width" style="grid-column: span 2;">
                                <label>Observa\xE7\xF5es Financeiras</label>
...
async function saveNFManual() {
  const payload = {
    ...
    // Mantido como valor da NF para compatibilidade
    // Novos campos financeiros (preparação futura)
    financeiro_info: {
  ...
  if (!payload.numero_nf || !payload.fornecedor_nome) {
    showToast("N\xBA NF e Fornecedor s\xE3o obrigat\xF3rios", "warning");
    return;
  }
...
async function renderNFDetail(id) {
  ...
  if (!nf) {
    showToast("Nota n\xE3o encontrada", "error");
    renderNFAbertasList();
    return;
  }
  app.innerHTML = `
                ...
                <div style="padding: 12px 20px 40px 20px;">
                    <div style="background: white; border-radius: 24px; padding: 24px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; border-bottom: 1px solid #f0f0f0; padding-bottom: 15px;">
                            <div>
                                <div style="font-size: 0.6rem; color: #999; text-transform: uppercase; font-weight: 700;">N\xFAmero / S\xE9rie</div>
                        ...
                        <div style="margin-top: 24px; padding-top: 16px; border-top: 1px dashed #eee;">
                            ...
                            <div class="form-grid" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px;">
                                ...
                                <div class="input-group">
                                    <label style="color: #999; font-size: 0.6rem;">FORMA / CONDI\xC7\xC3O</label>
                                    <div style="font-weight: 600; color: #444; font-size: 0.8rem; text-transform: uppercase;">
                                        ${nf.pagamento_forma || "-"} / ${nf.pagamento_condicao === "a_vista" ? "\xC0 VISTA" : nf.pagamento_parcelas + "x PARCELADO"}
                                    </div>
                                </div>
                                <div class="input-group">
                                    <label style="color: #999; font-size: 0.6rem;">1\xBA VENCIMENTO</label>
                                    <div style="font-weight: 600; color: #444; font-size: 0.8rem;">
                                        ${nf.pagamento_vencimento ? (/* @__PURE__ */ new Date(nf.pagamento_vencimento + "T00:00:00")).toLocaleDateString("pt-BR") : "-"}
                                    </div>
                                </div>
                            </div>
                            ${nf.observacoes_financeiras ? `
                                <div style="margin-top: 12px; padding: 10px; background: #f8f9fa; border-radius: 8px; border-left: 3px solid #ddd;">
                                    <label style="color: #999; font-size: 0.6rem; display: block; margin-bottom: 4px;">OBSERVA\xC7\xD5ES FINANCEIRAS</label>
                                    <div style="font-size: 0.75rem; color: #666; line-height: 1.4;">${nf.observacoes_financeiras}</div>
                                </div>
                            ` : ""}
                        </div>

                        <!-- SE\xC7\xC3O DE ITENS (PREPARA\xC7\xC3O FASE 2) -->
                        <div style="margin-top: 30px; border-top: 2px solid #f0f0f0; padding-top: 20px;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                                ...
                                <button class="btn-action" onclick="showToast('Adi\xE7\xE3o de itens em desenvolvimento (Fase 2)', 'info')" style="padding: 8px 16px; font-size: 0.75rem; background: #000 !important;">
        ...
        <div class="dashboard-screen internal fade-in nf-placeholder-screen entrada-nf-screen">
            ...
            <main class="container" style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 60vh;">
                <div style="text-align: center; padding: 40px; background: rgba(255,255,255,0.03); border-radius: 32px; border: 1px dashed rgba(255,255,255,0.1); max-width: 400px; width: 90%;">
                    ...
                    <p style="color: rgba(255,255,255,0.3); font-size: 0.8rem; margin-top: 10px;">Esta funcionalidade estar\xE1 dispon\xEDvel em breve.</p>
                </div>
            </main>
        </div>
    `;
